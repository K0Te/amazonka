...
data PutIntegration = PutIntegration'{_pRequestTemplates
                                      :: !(Maybe (Map Text Text)),
...
--
-- * 'pTlsConfig' - Undocumented member.
--
-- * 'pCacheNamespace' - Specifies a group of related cached parameters. By default, API Gateway uses the resource ID as the @cacheNamespace@ . You can specify the same @cacheNamespace@ across resources to return the same cached data for requests to different resources.
--
...
--
-- * 'pCacheKeyParameters' - A list of request parameters whose values API Gateway caches. To be valid values for @cacheKeyParameters@ , these parameters must also be specified for 'Method' @requestParameters@ .
--
...
putIntegration
    ...
    -> PutIntegration
putIntegration pRestAPIId_ pResourceId_ pHttpMethod_
  pType_
  = PutIntegration'{_pRequestTemplates = Nothing,
                    _pCredentials = Nothing, _pConnectionId = Nothing,
                    _pRequestParameters = Nothing,
                    _pContentHandling = Nothing,
                    _pPassthroughBehavior = Nothing, _pUri = Nothing,
                    _pTlsConfig = Nothing, _pCacheNamespace = Nothing,
                    ...
                    _pHttpMethod = pHttpMethod_, _pType = pType_}
...
-- | Undocumented member.
pTlsConfig :: Lens' PutIntegration (Maybe TLSConfig)
pTlsConfig = lens _pTlsConfig (\ s a -> s{_pTlsConfig = a})
