...
instance AWSPager DescribeBudgetPerformanceHistory
         where
        page rq rs
          | stop (rs ^. dbphrsNextToken) = Nothing
          | stop (rs ^. dbphrsBudgetPerformanceHistory) =
            Nothing
          | otherwise =
            Just $ rq & dbphNextToken .~ rs ^. dbphrsNextToken
