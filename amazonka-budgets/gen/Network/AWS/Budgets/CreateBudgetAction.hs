...
data CreateBudgetAction = CreateBudgetAction'{_cbaAccountId
                                              :: !Text,
                                              _cbaBudgetName :: !Text,
...
createBudgetAction
    :: Text -- ^ 'cbaAccountId'
    -> Text -- ^ 'cbaBudgetName'
    -> NotificationType -- ^ 'cbaNotificationType'
    -> ActionType -- ^ 'cbaActionType'
    -> ActionThreshold -- ^ 'cbaActionThreshold'
    -> Definition -- ^ 'cbaDefinition'
    -> Text -- ^ 'cbaExecutionRoleARN'
    -> ApprovalModel -- ^ 'cbaApprovalModel'
    -> NonEmpty Subscriber -- ^ 'cbaSubscribers'
    -> CreateBudgetAction
createBudgetAction pAccountId_ pBudgetName_
  pNotificationType_ pActionType_ pActionThreshold_
  pDefinition_ pExecutionRoleARN_ pApprovalModel_
  pSubscribers_
  = CreateBudgetAction'{_cbaAccountId = pAccountId_,
...
instance AWSRequest CreateBudgetAction where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 CreateBudgetActionResponse' <$>
                   (pure (fromEnum s)) <*> (x .:> "AccountId") <*>
                     (x .:> "BudgetName")
                     <*> (x .:> "ActionId"))

instance Hashable CreateBudgetAction where

instance NFData CreateBudgetAction where
...
data CreateBudgetActionResponse = CreateBudgetActionResponse'{_cbarsResponseStatus
                                                              :: !Int,
                                                              _cbarsAccountId ::
                                                              !Text,
                                                              _cbarsBudgetName
...
createBudgetActionResponse
    :: Int -- ^ 'cbarsResponseStatus'
    -> Text -- ^ 'cbarsAccountId'
    -> Text -- ^ 'cbarsBudgetName'
    -> Text -- ^ 'cbarsActionId'
    -> CreateBudgetActionResponse
createBudgetActionResponse pResponseStatus_
  pAccountId_ pBudgetName_ pActionId_
  = CreateBudgetActionResponse'{_cbarsResponseStatus =
                                  pResponseStatus_,
...
instance NFData CreateBudgetActionResponse where
