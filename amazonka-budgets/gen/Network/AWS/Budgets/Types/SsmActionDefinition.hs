...
data SsmActionDefinition = SsmActionDefinition'{_sadActionSubType
                                                :: !ActionSubType,
                                                _sadRegion :: !Text,
                                                _sadInstanceIds ::
                                                !(List1 Text)}
...
ssmActionDefinition
    :: ActionSubType -- ^ 'sadActionSubType'
    -> Text -- ^ 'sadRegion'
    -> NonEmpty Text -- ^ 'sadInstanceIds'
    -> SsmActionDefinition
ssmActionDefinition pActionSubType_ pRegion_
  pInstanceIds_
  = SsmActionDefinition'{_sadActionSubType =
                           pActionSubType_,
                         _sadRegion = pRegion_,
...
instance FromJSON SsmActionDefinition where
        parseJSON
          = withObject "SsmActionDefinition"
              (\ x ->
                 SsmActionDefinition' <$>
                   (x .: "ActionSubType") <*> (x .: "Region") <*>
                     (x .: "InstanceIds"))

instance Hashable SsmActionDefinition where

instance NFData SsmActionDefinition where
