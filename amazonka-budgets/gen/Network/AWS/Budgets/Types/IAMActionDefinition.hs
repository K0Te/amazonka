...
data IAMActionDefinition = IAMActionDefinition'{_iadGroups
                                                :: !(Maybe (List1 Text)),
                                                _iadRoles ::
                                                !(Maybe (List1 Text)),
                                                _iadUsers ::
...
iamActionDefinition pPolicyARN_
  = IAMActionDefinition'{_iadGroups = Nothing,
                         _iadRoles = Nothing, _iadUsers = Nothing,
...
instance FromJSON IAMActionDefinition where
        parseJSON
          = withObject "IAMActionDefinition"
              (\ x ->
                 IAMActionDefinition' <$>
                   (x .:? "Groups") <*> (x .:? "Roles") <*>
                     (x .:? "Users")
...
instance Hashable IAMActionDefinition where

instance NFData IAMActionDefinition where
