...
data ListBuildBatches = ListBuildBatches'{_lbbSortOrder
                                          :: !(Maybe SortOrderType),
...
listBuildBatches
    :: ListBuildBatches
listBuildBatches
  = ListBuildBatches'{_lbbSortOrder = Nothing,
                      _lbbNextToken = Nothing, _lbbFilter = Nothing,
...
instance AWSPager ListBuildBatches where
        page rq rs
          ...
          | otherwise =
            Just $ rq & lbbNextToken .~ rs ^. lbbrsNextToken
...
instance AWSRequest ListBuildBatches where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 ListBuildBatchesResponse' <$>
                   (x .?> "ids" .!@ mempty) <*> (x .?> "nextToken") <*>
                     (pure (fromEnum s)))

instance Hashable ListBuildBatches where

instance NFData ListBuildBatches where
...
data ListBuildBatchesResponse = ListBuildBatchesResponse'{_lbbrsIds
                                                          :: !(Maybe [Text]),
                                                          _lbbrsNextToken ::
                                                          !(Maybe Text),
                                                          _lbbrsResponseStatus
                                                          :: !Int}
...
listBuildBatchesResponse
    :: Int -- ^ 'lbbrsResponseStatus'
    -> ListBuildBatchesResponse
listBuildBatchesResponse pResponseStatus_
  = ListBuildBatchesResponse'{_lbbrsIds = Nothing,
...
instance NFData ListBuildBatchesResponse where
