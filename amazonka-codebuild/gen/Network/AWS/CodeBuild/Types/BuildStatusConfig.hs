...
data BuildStatusConfig = BuildStatusConfig'{_bscContext
                                            :: !(Maybe Text),
...
buildStatusConfig
    :: BuildStatusConfig
buildStatusConfig
  = BuildStatusConfig'{_bscContext = Nothing,
...
instance FromJSON BuildStatusConfig where
        parseJSON
          = withObject "BuildStatusConfig"
              (\ x ->
                 BuildStatusConfig' <$>
                   (x .:? "context") <*> (x .:? "targetUrl"))

instance Hashable BuildStatusConfig where

instance NFData BuildStatusConfig where
