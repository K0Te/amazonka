...
data BatchRestrictions = BatchRestrictions'{_brMaximumBuildsAllowed
                                            :: !(Maybe Int),
...
batchRestrictions
    :: BatchRestrictions
batchRestrictions
  = BatchRestrictions'{_brMaximumBuildsAllowed =
                         Nothing,
...
instance FromJSON BatchRestrictions where
        parseJSON
          = withObject "BatchRestrictions"
              (\ x ->
                 BatchRestrictions' <$>
                   (x .:? "maximumBuildsAllowed") <*>
                     (x .:? "computeTypesAllowed" .!= mempty))

instance Hashable BatchRestrictions where

instance NFData BatchRestrictions where
