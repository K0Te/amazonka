...
data ReportWithRawData = ReportWithRawData'{_rwrdData
                                            :: !(Maybe Text),
...
reportWithRawData
    :: ReportWithRawData
reportWithRawData
  = ReportWithRawData'{_rwrdData = Nothing,
...
instance FromJSON ReportWithRawData where
        parseJSON
          = withObject "ReportWithRawData"
              (\ x ->
                 ReportWithRawData' <$>
                   (x .:? "data") <*> (x .:? "reportArn"))

instance Hashable ReportWithRawData where

instance NFData ReportWithRawData where
