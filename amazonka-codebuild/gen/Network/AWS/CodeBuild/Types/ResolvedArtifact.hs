...
data ResolvedArtifact = ResolvedArtifact'{_raLocation
                                          :: !(Maybe Text),
...
resolvedArtifact
    :: ResolvedArtifact
resolvedArtifact
  = ResolvedArtifact'{_raLocation = Nothing,
                      _raIdentifier = Nothing, _raType = Nothing}
...
instance FromJSON ResolvedArtifact where
        parseJSON
          = withObject "ResolvedArtifact"
              (\ x ->
                 ResolvedArtifact' <$>
                   (x .:? "location") <*> (x .:? "identifier") <*>
                     (x .:? "type"))

instance Hashable ResolvedArtifact where

instance NFData ResolvedArtifact where
