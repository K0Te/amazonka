...
data GetReportGroupTrend = GetReportGroupTrend'{_grgtNumOfReports
                                                :: !(Maybe Nat),
...
getReportGroupTrend
    :: Text -- ^ 'grgtReportGroupARN'
    -> ReportGroupTrendFieldType -- ^ 'grgtTrendField'
    -> GetReportGroupTrend
getReportGroupTrend pReportGroupARN_ pTrendField_
  = GetReportGroupTrend'{_grgtNumOfReports = Nothing,
...
instance AWSRequest GetReportGroupTrend where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 GetReportGroupTrendResponse' <$>
                   (x .?> "rawData" .!@ mempty) <*> (x .?> "stats") <*>
                     (pure (fromEnum s)))

instance Hashable GetReportGroupTrend where

instance NFData GetReportGroupTrend where
...
data GetReportGroupTrendResponse = GetReportGroupTrendResponse'{_grgtrsRawData
                                                                ::
                                                                !(Maybe
                                                                    [ReportWithRawData]),
                                                                _grgtrsStats ::
...
getReportGroupTrendResponse
    :: Int -- ^ 'grgtrsResponseStatus'
    -> GetReportGroupTrendResponse
getReportGroupTrendResponse pResponseStatus_
  = GetReportGroupTrendResponse'{_grgtrsRawData =
                                   Nothing,
...
instance NFData GetReportGroupTrendResponse where
