...
data EquipmentDetection = EquipmentDetection'{_edBoundingBox
                                              :: !(Maybe BoundingBox),
                                              _edCoversBodyPart ::
...
equipmentDetection
    :: EquipmentDetection
equipmentDetection
  = EquipmentDetection'{_edBoundingBox = Nothing,
                        _edCoversBodyPart = Nothing, _edConfidence = Nothing,
                        _edType = Nothing}
...
instance FromJSON EquipmentDetection where
        parseJSON
          = withObject "EquipmentDetection"
              (\ x ->
                 EquipmentDetection' <$>
                   (x .:? "BoundingBox") <*> (x .:? "CoversBodyPart")
                     <*> (x .:? "Confidence")
                     <*> (x .:? "Type"))

instance Hashable EquipmentDetection where

instance NFData EquipmentDetection where
