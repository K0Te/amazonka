...
data AudioMetadata = AudioMetadata'{_amCodec ::
                                    !(Maybe Text),
...
audioMetadata
    :: AudioMetadata
audioMetadata
  = AudioMetadata'{_amCodec = Nothing,
...
instance FromJSON AudioMetadata where
        parseJSON
          = withObject "AudioMetadata"
              (\ x ->
                 AudioMetadata' <$>
                   (x .:? "Codec") <*> (x .:? "SampleRate") <*>
                     (x .:? "NumberOfChannels")
                     <*> (x .:? "DurationMillis"))

instance Hashable AudioMetadata where

instance NFData AudioMetadata where
