...
data SegmentTypeInfo = SegmentTypeInfo'{_stiModelVersion
                                        :: !(Maybe Text),
...
segmentTypeInfo
    :: SegmentTypeInfo
segmentTypeInfo
  = SegmentTypeInfo'{_stiModelVersion = Nothing,
                     _stiType = Nothing}
...
instance FromJSON SegmentTypeInfo where
        parseJSON
          = withObject "SegmentTypeInfo"
              (\ x ->
                 SegmentTypeInfo' <$>
                   (x .:? "ModelVersion") <*> (x .:? "Type"))

instance Hashable SegmentTypeInfo where

instance NFData SegmentTypeInfo where
