...
data CoversBodyPart = CoversBodyPart'{_cbpValue ::
                                      !(Maybe Bool),
...
coversBodyPart
    :: CoversBodyPart
coversBodyPart
  = CoversBodyPart'{_cbpValue = Nothing,
...
instance FromJSON CoversBodyPart where
        parseJSON
          = withObject "CoversBodyPart"
              (\ x ->
                 CoversBodyPart' <$>
                   (x .:? "Value") <*> (x .:? "Confidence"))

instance Hashable CoversBodyPart where

instance NFData CoversBodyPart where
