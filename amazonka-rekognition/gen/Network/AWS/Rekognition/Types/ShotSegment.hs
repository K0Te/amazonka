...
data ShotSegment = ShotSegment'{_ssConfidence ::
                                !(Maybe Double),
                                _ssIndex :: !(Maybe Nat)}
...
shotSegment
    :: ShotSegment
shotSegment
  = ShotSegment'{_ssConfidence = Nothing,
                 _ssIndex = Nothing}
...
instance FromJSON ShotSegment where
        parseJSON
          = withObject "ShotSegment"
              (\ x ->
                 ShotSegment' <$>
                   (x .:? "Confidence") <*> (x .:? "Index"))

instance Hashable ShotSegment where

instance NFData ShotSegment where
