...
data RegionsInfo = RegionsInfo'{_riPrimaryRegion ::
                                !(Maybe Text),
...
regionsInfo
    :: RegionsInfo
regionsInfo
  = RegionsInfo'{_riPrimaryRegion = Nothing,
...
instance FromJSON RegionsInfo where
        parseJSON
          = withObject "RegionsInfo"
              (\ x ->
                 RegionsInfo' <$>
                   (x .:? "PrimaryRegion") <*>
                     (x .:? "AdditionalRegions" .!= mempty))

instance Hashable RegionsInfo where

instance NFData RegionsInfo where
