...
data CertificateInfo = CertificateInfo'{_ciState ::
...
certificateInfo
    :: CertificateInfo
certificateInfo
  = CertificateInfo'{_ciState = Nothing,
                     _ciCommonName = Nothing, _ciCertificateId = Nothing,
                     _ciExpiryDateTime = Nothing, _ciType = Nothing}
...
instance FromJSON CertificateInfo where
        parseJSON
          = withObject "CertificateInfo"
              (\ x ->
                 CertificateInfo' <$>
                   (x .:? "State") <*> (x .:? "CommonName") <*>
                     (x .:? "CertificateId")
                     <*> (x .:? "ExpiryDateTime")
                     <*> (x .:? "Type"))

instance Hashable CertificateInfo where

instance NFData CertificateInfo where
