...
stackImportComplete
  = Wait{_waitName = "StackImportComplete",
         ...
         _waitAcceptors =
           [matchAll "IMPORT_COMPLETE" AcceptSuccess
              (folding (concatOf dsrsStacks) . staStackStatus .
                 to toTextCI),
            ...
            matchAny "ROLLBACK_FAILED" AcceptFailure
              (folding (concatOf dsrsStacks) . staStackStatus .
                 to toTextCI),
            matchAny "IMPORT_ROLLBACK_IN_PROGRESS" AcceptFailure
              (folding (concatOf dsrsStacks) . staStackStatus .
                 to toTextCI),
            matchAny "IMPORT_ROLLBACK_FAILED" AcceptFailure
              (folding (concatOf dsrsStacks) . staStackStatus .
                 to toTextCI),
            matchAny "IMPORT_ROLLBACK_COMPLETE" AcceptFailure
              (folding (concatOf dsrsStacks) . staStackStatus .
                 to toTextCI),
...
stackRollbackComplete
  = Wait{_waitName = "StackRollbackComplete",
         _waitAttempts = 120, _waitDelay = 30,
         _waitAcceptors =
           [matchAll "UPDATE_ROLLBACK_COMPLETE" AcceptSuccess
              (folding (concatOf dsrsStacks) . staStackStatus .
                 to toTextCI),
...
stackExists
  = Wait{_waitName = "StackExists", _waitAttempts = 20,
         _waitDelay = 5,
         _waitAcceptors =
