...
newtype CancelInstanceRefresh = CancelInstanceRefresh'{_cirAutoScalingGroupName
                                                       :: Text}
...
cancelInstanceRefresh
    :: Text -- ^ 'cirAutoScalingGroupName'
    -> CancelInstanceRefresh
cancelInstanceRefresh pAutoScalingGroupName_
  = CancelInstanceRefresh'{_cirAutoScalingGroupName =
...
instance AWSRequest CancelInstanceRefresh where
        type Rs CancelInstanceRefresh =
             CancelInstanceRefreshResponse
        request = postQuery autoScaling
        response
          = receiveXMLWrapper "CancelInstanceRefreshResult"
              (\ s h x ->
                 CancelInstanceRefreshResponse' <$>
                   (x .@? "InstanceRefreshId") <*> (pure (fromEnum s)))

instance Hashable CancelInstanceRefresh where

instance NFData CancelInstanceRefresh where
...
data CancelInstanceRefreshResponse = CancelInstanceRefreshResponse'{_cirrsInstanceRefreshId
                                                                    ::
                                                                    !(Maybe
                                                                        Text),
...
cancelInstanceRefreshResponse pResponseStatus_
  = CancelInstanceRefreshResponse'{_cirrsInstanceRefreshId
                                     = Nothing,
...
instance NFData CancelInstanceRefreshResponse where
