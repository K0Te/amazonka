...
newtype ListStudios = ListStudios'{_lMarker ::
                                   Maybe Text}
...
instance AWSRequest ListStudios where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 ListStudiosResponse' <$>
                   (x .?> "Studios" .!@ mempty) <*> (x .?> "Marker") <*>
                     (pure (fromEnum s)))

instance Hashable ListStudios where

instance NFData ListStudios where
...
data ListStudiosResponse = ListStudiosResponse'{_lsrsStudios
                                                :: !(Maybe [StudioSummary]),
...
listStudiosResponse pResponseStatus_
  = ListStudiosResponse'{_lsrsStudios = Nothing,
                         _lsrsMarker = Nothing,
...
instance NFData ListStudiosResponse where
