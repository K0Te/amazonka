...
data SessionMappingSummary = SessionMappingSummary'{_smsCreationTime
                                                    :: !(Maybe POSIX),
                                                    _smsStudioId ::
                                                    !(Maybe Text),
                                                    _smsIdentityType ::
...
sessionMappingSummary
    :: SessionMappingSummary
sessionMappingSummary
  = SessionMappingSummary'{_smsCreationTime = Nothing,
                           _smsStudioId = Nothing, _smsIdentityType = Nothing,
...
instance FromJSON SessionMappingSummary where
        parseJSON
          = withObject "SessionMappingSummary"
              (\ x ->
                 SessionMappingSummary' <$>
                   (x .:? "CreationTime") <*> (x .:? "StudioId") <*>
                     (x .:? "IdentityType")
                     <*> (x .:? "IdentityId")
                     <*> (x .:? "SessionPolicyArn")
                     <*> (x .:? "IdentityName"))

instance Hashable SessionMappingSummary where

instance NFData SessionMappingSummary where
