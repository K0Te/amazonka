...
newtype ManagedScalingPolicy = ManagedScalingPolicy'{_mspComputeLimits
                                                     :: Maybe ComputeLimits}
...
instance FromJSON ManagedScalingPolicy where
        parseJSON
          = withObject "ManagedScalingPolicy"
              (\ x ->
                 ManagedScalingPolicy' <$> (x .:? "ComputeLimits"))

instance Hashable ManagedScalingPolicy where

instance NFData ManagedScalingPolicy where
