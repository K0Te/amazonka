...
data Sign = Sign'{_sMessageType ::
                  !(Maybe MessageType),
                  _sGrantTokens :: !(Maybe [Text]), _sKeyId :: !Text,
...
sign
    :: Text -- ^ 'sKeyId'
    -> ByteString -- ^ 'sMessage'
    -> SigningAlgorithmSpec -- ^ 'sSigningAlgorithm'
    -> Sign
sign pKeyId_ pMessage_ pSigningAlgorithm_
  = Sign'{_sMessageType = Nothing,
          _sGrantTokens = Nothing, _sKeyId = pKeyId_,
...
instance AWSRequest Sign where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 SignResponse' <$>
                   (x .?> "SigningAlgorithm") <*> (x .?> "Signature")
                     <*> (x .?> "KeyId")
                     <*> (pure (fromEnum s)))
...
data SignResponse = SignResponse'{_srsSigningAlgorithm
...
signResponse pResponseStatus_
  = SignResponse'{_srsSigningAlgorithm = Nothing,
                  _srsSignature = Nothing, _srsKeyId = Nothing,
...
instance NFData SignResponse where
