...
data StatefulRuleGroup = StatefulRuleGroup'{_srgResourceId
                                            :: !(Maybe Text),
...
statefulRuleGroup
    :: StatefulRuleGroup
statefulRuleGroup
  = StatefulRuleGroup'{_srgResourceId = Nothing,
...
instance FromJSON StatefulRuleGroup where
        parseJSON
          = withObject "StatefulRuleGroup"
              (\ x ->
                 StatefulRuleGroup' <$>
                   (x .:? "ResourceId") <*> (x .:? "RuleGroupName"))

instance Hashable StatefulRuleGroup where

instance NFData StatefulRuleGroup where
