...
data SecurityGroupRuleDescription = SecurityGroupRuleDescription'{_sgrdFromPort
                                                                  ::
                                                                  !(Maybe Nat),
                                                                  _sgrdProtocol
                                                                  ::
...
securityGroupRuleDescription
    :: SecurityGroupRuleDescription
securityGroupRuleDescription
  = SecurityGroupRuleDescription'{_sgrdFromPort =
                                    Nothing,
...
instance FromJSON SecurityGroupRuleDescription where
        parseJSON
          = withObject "SecurityGroupRuleDescription"
              (\ x ->
                 SecurityGroupRuleDescription' <$>
                   (x .:? "FromPort") <*> (x .:? "Protocol") <*>
                     (x .:? "IPV4Range")
                     <*> (x .:? "PrefixListId")
                     <*> (x .:? "ToPort")
                     <*> (x .:? "IPV6Range"))

instance Hashable SecurityGroupRuleDescription where

instance NFData SecurityGroupRuleDescription where
