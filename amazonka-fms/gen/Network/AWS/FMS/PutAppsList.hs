...
data PutAppsList = PutAppsList'{_palTagList ::
                                !(Maybe [Tag]),
...
putAppsList
    :: AppsListData -- ^ 'palAppsList'
    -> PutAppsList
putAppsList pAppsList_
  = PutAppsList'{_palTagList = Nothing,
...
instance AWSRequest PutAppsList where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 PutAppsListResponse' <$>
                   (x .?> "AppsListArn") <*> (x .?> "AppsList") <*>
                     (pure (fromEnum s)))

instance Hashable PutAppsList where

instance NFData PutAppsList where
...
data PutAppsListResponse = PutAppsListResponse'{_palrsAppsListARN
                                                :: !(Maybe Text),
                                                _palrsAppsList ::
                                                !(Maybe AppsListData),
...
putAppsListResponse
    :: Int -- ^ 'palrsResponseStatus'
    -> PutAppsListResponse
putAppsListResponse pResponseStatus_
  = PutAppsListResponse'{_palrsAppsListARN = Nothing,
...
instance NFData PutAppsListResponse where
