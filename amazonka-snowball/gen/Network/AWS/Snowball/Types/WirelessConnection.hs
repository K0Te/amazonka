...
newtype WirelessConnection = WirelessConnection'{_wcIsWifiEnabled
                                                 :: Maybe Bool}
...
instance FromJSON WirelessConnection where
        parseJSON
          = withObject "WirelessConnection"
              (\ x ->
                 WirelessConnection' <$> (x .:? "IsWifiEnabled"))

instance Hashable WirelessConnection where

instance NFData WirelessConnection where
