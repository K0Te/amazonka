...
newtype GetFunctionConcurrency = GetFunctionConcurrency'{_gFunctionName
                                                         :: Text}
...
getFunctionConcurrency pFunctionName_
  = GetFunctionConcurrency'{_gFunctionName =
                              pFunctionName_}
...
instance AWSRequest GetFunctionConcurrency where
        type Rs GetFunctionConcurrency =
             GetFunctionConcurrencyResponse
        request = get lambda
        response
          = receiveJSON
...
instance Hashable GetFunctionConcurrency where

instance NFData GetFunctionConcurrency where
...
instance ToPath GetFunctionConcurrency where
        toPath GetFunctionConcurrency'{..}
          = mconcat
              ["/2019-09-30/functions/", toBS _gFunctionName,
               "/concurrency"]
...
data GetFunctionConcurrencyResponse = GetFunctionConcurrencyResponse'{_gfcrsReservedConcurrentExecutions
                                                                      ::
                                                                      !(Maybe
                                                                          Nat),
                                                                      _gfcrsResponseStatus
                                                                      :: !Int}
...
getFunctionConcurrencyResponse
    :: Int -- ^ 'gfcrsResponseStatus'
    -> GetFunctionConcurrencyResponse
getFunctionConcurrencyResponse pResponseStatus_
  = GetFunctionConcurrencyResponse'{_gfcrsReservedConcurrentExecutions
                                      = Nothing,
...
instance NFData GetFunctionConcurrencyResponse where
