...
--
-- * 'ufcMemorySize' - The amount of memory available to the function at runtime. Increasing the function's memory also increases its CPU allocation. The default value is 128 MB. The value can be any multiple of 1 MB.
--
...
--
-- * 'ufcImageConfig' - Configuration values that override the container image Dockerfile.
--
...
ufcImageConfig :: Lens' UpdateFunctionConfiguration (Maybe ImageConfig)
ufcImageConfig = lens _ufcImageConfig (\ s a -> s{_ufcImageConfig = a})
