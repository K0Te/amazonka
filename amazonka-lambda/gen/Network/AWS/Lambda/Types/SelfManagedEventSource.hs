...
newtype SelfManagedEventSource = SelfManagedEventSource'{_smesEndpoints
                                                         ::
                                                         Maybe
                                                           (Map EndPointType
                                                              (List1 Text))}
...
instance FromJSON SelfManagedEventSource where
        parseJSON
          = withObject "SelfManagedEventSource"
              (\ x ->
                 SelfManagedEventSource' <$>
                   (x .:? "Endpoints" .!= mempty))

instance Hashable SelfManagedEventSource where

instance NFData SelfManagedEventSource where
