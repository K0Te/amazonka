...
data FileSystemConfig = FileSystemConfig'{_fscARN ::
                                          !Text,
...
fileSystemConfig
    :: Text -- ^ 'fscARN'
    -> Text -- ^ 'fscLocalMountPath'
    -> FileSystemConfig
fileSystemConfig pARN_ pLocalMountPath_
  = FileSystemConfig'{_fscARN = pARN_,
...
instance FromJSON FileSystemConfig where
        parseJSON
          = withObject "FileSystemConfig"
              (\ x ->
                 FileSystemConfig' <$>
                   (x .: "Arn") <*> (x .: "LocalMountPath"))

instance Hashable FileSystemConfig where

instance NFData FileSystemConfig where
