...
newtype AllowedPublishers = AllowedPublishers'{_apSigningProfileVersionARNs
                                               :: List1 Text}
...
allowedPublishers
    :: NonEmpty Text -- ^ 'apSigningProfileVersionARNs'
    -> AllowedPublishers
allowedPublishers pSigningProfileVersionARNs_
  = AllowedPublishers'{_apSigningProfileVersionARNs =
...
instance FromJSON AllowedPublishers where
        parseJSON
          = withObject "AllowedPublishers"
              (\ x ->
                 AllowedPublishers' <$>
                   (x .: "SigningProfileVersionArns"))

instance Hashable AllowedPublishers where

instance NFData AllowedPublishers where
