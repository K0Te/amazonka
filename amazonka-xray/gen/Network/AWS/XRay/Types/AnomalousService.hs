...
newtype AnomalousService = AnomalousService'{_asServiceId
                                             :: Maybe ServiceId}
...
instance FromJSON AnomalousService where
        parseJSON
          = withObject "AnomalousService"
              (\ x -> AnomalousService' <$> (x .:? "ServiceId"))

instance Hashable AnomalousService where

instance NFData AnomalousService where
