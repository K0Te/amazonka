...
newtype InsightImpactGraphEdge = InsightImpactGraphEdge'{_iigeReferenceId
                                                         :: Maybe Int}
...
instance FromJSON InsightImpactGraphEdge where
        parseJSON
          = withObject "InsightImpactGraphEdge"
              (\ x ->
                 InsightImpactGraphEdge' <$> (x .:? "ReferenceId"))

instance Hashable InsightImpactGraphEdge where

instance NFData InsightImpactGraphEdge where
