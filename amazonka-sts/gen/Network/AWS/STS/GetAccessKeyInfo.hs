...
newtype GetAccessKeyInfo = GetAccessKeyInfo'{_gakiAccessKeyId
                                             :: AccessKey}
...
instance AWSRequest GetAccessKeyInfo where
        ...
        response
          = receiveXMLWrapper "GetAccessKeyInfoResult"
              (\ s h x ->
                 GetAccessKeyInfoResponse' <$>
                   (x .@? "Account") <*> (pure (fromEnum s)))

instance Hashable GetAccessKeyInfo where

instance NFData GetAccessKeyInfo where
...
data GetAccessKeyInfoResponse = GetAccessKeyInfoResponse'{_gakirsAccount
                                                          :: !(Maybe Text),
                                                          _gakirsResponseStatus
                                                          :: !Int}
...
getAccessKeyInfoResponse
    :: Int -- ^ 'gakirsResponseStatus'
    -> GetAccessKeyInfoResponse
getAccessKeyInfoResponse pResponseStatus_
  = GetAccessKeyInfoResponse'{_gakirsAccount = Nothing,
...
instance NFData GetAccessKeyInfoResponse where
