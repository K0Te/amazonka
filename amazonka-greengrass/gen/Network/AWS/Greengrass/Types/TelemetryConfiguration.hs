...
data TelemetryConfiguration = TelemetryConfiguration'{_tcConfigurationSyncStatus
                                                      ::
                                                      !(Maybe
...
telemetryConfiguration pTelemetry_
  = TelemetryConfiguration'{_tcConfigurationSyncStatus
                              = Nothing,
...
instance FromJSON TelemetryConfiguration where
        parseJSON
          = withObject "TelemetryConfiguration"
              (\ x ->
                 TelemetryConfiguration' <$>
                   (x .:? "ConfigurationSyncStatus") <*>
                     (x .: "Telemetry"))

instance Hashable TelemetryConfiguration where

instance NFData TelemetryConfiguration where
