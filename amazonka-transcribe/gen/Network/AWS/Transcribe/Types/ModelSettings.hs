...
newtype ModelSettings = ModelSettings'{_msLanguageModelName
                                       :: Maybe Text}
...
instance FromJSON ModelSettings where
        parseJSON
          = withObject "ModelSettings"
              (\ x ->
                 ModelSettings' <$> (x .:? "LanguageModelName"))

instance Hashable ModelSettings where

instance NFData ModelSettings where
