...
data DeleteJourney = DeleteJourney'{_djJourneyId ::
                                    !Text,
...
deleteJourney
    :: Text -- ^ 'djJourneyId'
    -> Text -- ^ 'djApplicationId'
    -> DeleteJourney
deleteJourney pJourneyId_ pApplicationId_
  = DeleteJourney'{_djJourneyId = pJourneyId_,
...
instance Hashable DeleteJourney where

instance NFData DeleteJourney where
...
instance ToPath DeleteJourney where
        toPath DeleteJourney'{..}
          = mconcat
              ["/v1/apps/", toBS _djApplicationId, "/journeys/",
               toBS _djJourneyId]
...
data DeleteJourneyResponse = DeleteJourneyResponse'{_djrsResponseStatus
                                                    :: !Int,
                                                    _djrsJourneyResponse ::
                                                    !JourneyResponse}
...
deleteJourneyResponse
    ...
    -> JourneyResponse -- ^ 'djrsJourneyResponse'
    -> DeleteJourneyResponse
deleteJourneyResponse pResponseStatus_
  pJourneyResponse_
  = DeleteJourneyResponse'{_djrsResponseStatus =
                             pResponseStatus_,
...
instance NFData DeleteJourneyResponse where
