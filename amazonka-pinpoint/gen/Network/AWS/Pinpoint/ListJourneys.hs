...
data ListJourneys = ListJourneys'{_ljToken ::
                                  !(Maybe Text),
...
listJourneys
    :: Text -- ^ 'ljApplicationId'
    -> ListJourneys
listJourneys pApplicationId_
  = ListJourneys'{_ljToken = Nothing,
                  _ljPageSize = Nothing,
...
instance Hashable ListJourneys where

instance NFData ListJourneys where
...
instance ToQuery ListJourneys where
        toQuery ListJourneys'{..}
          = mconcat
              ["token" =: _ljToken, "page-size" =: _ljPageSize]

-- | /See:/ 'listJourneysResponse' smart constructor.
data ListJourneysResponse = ListJourneysResponse'{_ljrsResponseStatus
                                                  :: !Int,
                                                  _ljrsJourneysResponse ::
                                                  !JourneysResponse}
                              deriving (Eq, Read, Show, Data, Typeable, Generic)
...
listJourneysResponse
    ...
    -> JourneysResponse -- ^ 'ljrsJourneysResponse'
    -> ListJourneysResponse
listJourneysResponse pResponseStatus_
  pJourneysResponse_
  = ListJourneysResponse'{_ljrsResponseStatus =
                            pResponseStatus_,
...
instance NFData ListJourneysResponse where
