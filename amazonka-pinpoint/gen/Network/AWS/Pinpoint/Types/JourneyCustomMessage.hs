...
newtype JourneyCustomMessage = JourneyCustomMessage'{_jcmData
                                                     :: Maybe Text}
...
instance FromJSON JourneyCustomMessage where
        parseJSON
          = withObject "JourneyCustomMessage"
              (\ x -> JourneyCustomMessage' <$> (x .:? "Data"))

instance Hashable JourneyCustomMessage where

instance NFData JourneyCustomMessage where
