...
data JourneyExecutionMetricsResponse = JourneyExecutionMetricsResponse'{_jemMetrics
                                                                        ::
                                                                        !(Map
                                                                            Text
                                                                            Text),
...
journeyExecutionMetricsResponse
    :: Text -- ^ 'jemJourneyId'
    -> Text -- ^ 'jemLastEvaluatedTime'
    -> Text -- ^ 'jemApplicationId'
    -> JourneyExecutionMetricsResponse
journeyExecutionMetricsResponse pJourneyId_
  pLastEvaluatedTime_ pApplicationId_
  = JourneyExecutionMetricsResponse'{_jemMetrics =
                                       mempty,
...
instance FromJSON JourneyExecutionMetricsResponse
         where
        parseJSON
          = withObject "JourneyExecutionMetricsResponse"
              (\ x ->
                 JourneyExecutionMetricsResponse' <$>
                   (x .:? "Metrics" .!= mempty) <*> (x .: "JourneyId")
                     <*> (x .: "LastEvaluatedTime")
                     <*> (x .: "ApplicationId"))
...
instance NFData JourneyExecutionMetricsResponse where
