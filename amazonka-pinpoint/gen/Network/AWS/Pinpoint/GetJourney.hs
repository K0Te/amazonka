...
data GetJourney = GetJourney'{_gjJourneyId :: !Text,
...
getJourney
    :: Text -- ^ 'gjJourneyId'
    -> Text -- ^ 'gjApplicationId'
    -> GetJourney
getJourney pJourneyId_ pApplicationId_
  = GetJourney'{_gjJourneyId = pJourneyId_,
...
instance Hashable GetJourney where

instance NFData GetJourney where
...
instance ToPath GetJourney where
        toPath GetJourney'{..}
          = mconcat
              ["/v1/apps/", toBS _gjApplicationId, "/journeys/",
               toBS _gjJourneyId]
...
data GetJourneyResponse = GetJourneyResponse'{_gjrsResponseStatus
                                              :: !Int,
                                              _gjrsJourneyResponse ::
                                              !JourneyResponse}
                            deriving (Eq, Read, Show, Data, Typeable, Generic)
...
getJourneyResponse
    ...
    -> JourneyResponse -- ^ 'gjrsJourneyResponse'
    -> GetJourneyResponse
getJourneyResponse pResponseStatus_ pJourneyResponse_
  = GetJourneyResponse'{_gjrsResponseStatus =
                          pResponseStatus_,
...
instance NFData GetJourneyResponse where
