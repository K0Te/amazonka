...
data UpdateJourney = UpdateJourney'{_ujJourneyId ::
                                    !Text,
...
updateJourney
    :: Text -- ^ 'ujJourneyId'
    -> Text -- ^ 'ujApplicationId'
    -> WriteJourneyRequest -- ^ 'ujWriteJourneyRequest'
    -> UpdateJourney
updateJourney pJourneyId_ pApplicationId_
  pWriteJourneyRequest_
  = UpdateJourney'{_ujJourneyId = pJourneyId_,
...
instance Hashable UpdateJourney where

instance NFData UpdateJourney where
...
instance ToPath UpdateJourney where
        toPath UpdateJourney'{..}
          = mconcat
              ["/v1/apps/", toBS _ujApplicationId, "/journeys/",
               toBS _ujJourneyId]
...
data UpdateJourneyResponse = UpdateJourneyResponse'{_ujrsResponseStatus
                                                    :: !Int,
                                                    _ujrsJourneyResponse ::
                                                    !JourneyResponse}
...
updateJourneyResponse
    ...
    -> JourneyResponse -- ^ 'ujrsJourneyResponse'
    -> UpdateJourneyResponse
updateJourneyResponse pResponseStatus_
  pJourneyResponse_
  = UpdateJourneyResponse'{_ujrsResponseStatus =
                             pResponseStatus_,
...
instance NFData UpdateJourneyResponse where
