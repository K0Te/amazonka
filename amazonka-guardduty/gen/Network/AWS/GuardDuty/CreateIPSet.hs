...
data CreateIPSet = CreateIPSet'{_cisClientToken ::
                                !(Maybe Text),
                                _cisTags :: !(Maybe (Map Text Text)),
                                _cisDetectorId :: !Text, _cisName :: !Text,
                                _cisFormat :: !IPSetFormat,
                                _cisLocation :: !Text, _cisActivate :: !Bool}
...
createIPSet
    ...
    -> Bool -- ^ 'cisActivate'
    -> CreateIPSet
createIPSet pDetectorId_ pName_ pFormat_ pLocation_
  pActivate_
  = CreateIPSet'{_cisClientToken = Nothing,
                 _cisTags = Nothing, _cisDetectorId = pDetectorId_,
                 _cisName = pName_, _cisFormat = pFormat_,
                 _cisLocation = pLocation_, _cisActivate = pActivate_}
          ...
          = object
              (catMaybes
                 [("clientToken" .=) <$> _cisClientToken,
                  ("tags" .=) <$> _cisTags, Just ("name" .= _cisName),
...
data CreateIPSetResponse = CreateIPSetResponse'{_cisrsResponseStatus
                                                :: !Int,
                                                _cisrsIPSetId :: !Text}
                             deriving (Eq, Read, Show, Data, Typeable, Generic)
...
createIPSetResponse pResponseStatus_ pIPSetId_
  = CreateIPSetResponse'{_cisrsResponseStatus =
                           pResponseStatus_,
