...
newtype DescribeOrganizationConfiguration = DescribeOrganizationConfiguration'{_docDetectorId
                                                                               ::
                                                                               Text}
...
describeOrganizationConfiguration pDetectorId_
  = DescribeOrganizationConfiguration'{_docDetectorId =
                                         pDetectorId_}
...
instance AWSRequest DescribeOrganizationConfiguration
         where
...
instance ToHeaders DescribeOrganizationConfiguration
         where
        toHeaders
          = const
              (mconcat
...
instance ToPath DescribeOrganizationConfiguration
         where
        toPath DescribeOrganizationConfiguration'{..}
          = mconcat
              ["/detector/", toBS _docDetectorId, "/admin"]

instance ToQuery DescribeOrganizationConfiguration
         where
        toQuery = const mempty

-- | /See:/ 'describeOrganizationConfigurationResponse' smart constructor.
data DescribeOrganizationConfigurationResponse = DescribeOrganizationConfigurationResponse'{_docrsDataSources
                                                                                            ::
                                                                                            !(Maybe
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'docrsDataSources' - Describes which data sources are enabled automatically for member accounts.
--
...
--
-- * 'docrsMemberAccountLimitReached' - Indicates whether the maximum number of allowed member accounts are already associated with the delegated administrator account for your organization.
describeOrganizationConfigurationResponse
    :: Int -- ^ 'docrsResponseStatus'
    -> Bool -- ^ 'docrsAutoEnable'
    -> Bool -- ^ 'docrsMemberAccountLimitReached'
    -> DescribeOrganizationConfigurationResponse
describeOrganizationConfigurationResponse
  pResponseStatus_ pAutoEnable_
  pMemberAccountLimitReached_
  = DescribeOrganizationConfigurationResponse'{_docrsDataSources
                                                 = Nothing,
                                               _docrsResponseStatus =
                                                 pResponseStatus_,
