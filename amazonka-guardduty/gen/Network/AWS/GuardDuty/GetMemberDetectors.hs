...
data GetMemberDetectors = GetMemberDetectors'{_gmdDetectorId
                                              :: !Text,
...
getMemberDetectors
    :: Text -- ^ 'gmdDetectorId'
    -> NonEmpty Text -- ^ 'gmdAccountIds'
    -> GetMemberDetectors
getMemberDetectors pDetectorId_ pAccountIds_
  = GetMemberDetectors'{_gmdDetectorId = pDetectorId_,
...
instance AWSRequest GetMemberDetectors where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 GetMemberDetectorsResponse' <$>
                   (pure (fromEnum s)) <*> (x .:> "members") <*>
                     (x .?> "unprocessedAccounts" .!@ mempty))

instance Hashable GetMemberDetectors where

instance NFData GetMemberDetectors where
...
data GetMemberDetectorsResponse = GetMemberDetectorsResponse'{_gmdrsResponseStatus
                                                              :: !Int,
...
getMemberDetectorsResponse
    ...
    -> NonEmpty MemberDataSourceConfiguration -- ^ 'gmdrsMemberDataSourceConfigurations'
    -> GetMemberDetectorsResponse
getMemberDetectorsResponse pResponseStatus_
  pMemberDataSourceConfigurations_
  = GetMemberDetectorsResponse'{_gmdrsResponseStatus =
                                  pResponseStatus_,
                                _gmdrsMemberDataSourceConfigurations =
...
instance NFData GetMemberDetectorsResponse where
