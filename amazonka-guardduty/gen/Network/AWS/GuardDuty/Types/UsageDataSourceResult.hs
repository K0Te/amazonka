...
data UsageDataSourceResult = UsageDataSourceResult'{_udsrTotal
                                                    :: !(Maybe Total),
                                                    _udsrDataSource ::
                                                    !(Maybe DataSource)}
...
usageDataSourceResult
    :: UsageDataSourceResult
usageDataSourceResult
  = UsageDataSourceResult'{_udsrTotal = Nothing,
...
instance FromJSON UsageDataSourceResult where
        parseJSON
          = withObject "UsageDataSourceResult"
              (\ x ->
                 UsageDataSourceResult' <$>
                   (x .:? "total") <*> (x .:? "dataSource"))

instance Hashable UsageDataSourceResult where

instance NFData UsageDataSourceResult where
