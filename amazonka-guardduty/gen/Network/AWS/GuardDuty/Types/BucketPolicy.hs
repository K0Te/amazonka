...
data BucketPolicy = BucketPolicy'{_bpAllowsPublicWriteAccess
                                  :: !(Maybe Bool),
...
bucketPolicy
    :: BucketPolicy
bucketPolicy
  = BucketPolicy'{_bpAllowsPublicWriteAccess = Nothing,
...
instance FromJSON BucketPolicy where
        parseJSON
          = withObject "BucketPolicy"
              (\ x ->
                 BucketPolicy' <$>
                   (x .:? "allowsPublicWriteAccess") <*>
                     (x .:? "allowsPublicReadAccess"))

instance Hashable BucketPolicy where

instance NFData BucketPolicy where
