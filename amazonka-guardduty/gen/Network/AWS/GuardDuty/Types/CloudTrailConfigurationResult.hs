...
newtype CloudTrailConfigurationResult = CloudTrailConfigurationResult'{_ctcrStatus
                                                                       ::
                                                                       DataSourceStatus}
...
cloudTrailConfigurationResult
    :: DataSourceStatus -- ^ 'ctcrStatus'
    -> CloudTrailConfigurationResult
cloudTrailConfigurationResult pStatus_
  = CloudTrailConfigurationResult'{_ctcrStatus =
                                     pStatus_}
...
instance FromJSON CloudTrailConfigurationResult where
        parseJSON
          = withObject "CloudTrailConfigurationResult"
              (\ x ->
                 CloudTrailConfigurationResult' <$> (x .: "status"))

instance Hashable CloudTrailConfigurationResult where

instance NFData CloudTrailConfigurationResult where
