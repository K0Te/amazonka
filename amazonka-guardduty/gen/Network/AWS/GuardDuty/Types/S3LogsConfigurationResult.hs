...
newtype S3LogsConfigurationResult = S3LogsConfigurationResult'{_slcrStatus
                                                               ::
                                                               DataSourceStatus}
...
instance FromJSON S3LogsConfigurationResult where
        parseJSON
          = withObject "S3LogsConfigurationResult"
              (\ x ->
                 S3LogsConfigurationResult' <$> (x .: "status"))

instance Hashable S3LogsConfigurationResult where

instance NFData S3LogsConfigurationResult where
