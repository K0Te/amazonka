...
data PublicAccess = PublicAccess'{_paPermissionConfiguration
...
publicAccess
    :: PublicAccess
publicAccess
  = PublicAccess'{_paPermissionConfiguration = Nothing,
...
instance FromJSON PublicAccess where
        parseJSON
          = withObject "PublicAccess"
              (\ x ->
                 PublicAccess' <$>
                   (x .:? "permissionConfiguration") <*>
                     (x .:? "effectivePermission"))

instance Hashable PublicAccess where

instance NFData PublicAccess where
