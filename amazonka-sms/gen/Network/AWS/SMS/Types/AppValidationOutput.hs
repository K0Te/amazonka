...
newtype AppValidationOutput = AppValidationOutput'{_avoSsmOutput
                                                   :: Maybe SSMOutput}
...
instance FromJSON AppValidationOutput where
        parseJSON
          = withObject "AppValidationOutput"
              (\ x -> AppValidationOutput' <$> (x .:? "ssmOutput"))

instance Hashable AppValidationOutput where

instance NFData AppValidationOutput where
