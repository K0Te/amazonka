...
data SSMValidationParameters = SSMValidationParameters'{_ssmvpInstanceId
                                                        :: !(Maybe Text),
                                                        _ssmvpCommand ::
                                                        !(Maybe Text),
...
sSMValidationParameters
    :: SSMValidationParameters
sSMValidationParameters
  = SSMValidationParameters'{_ssmvpInstanceId =
                               Nothing,
                             _ssmvpCommand = Nothing,
                             _ssmvpExecutionTimeoutSeconds = Nothing,
                             _ssmvpScriptType = Nothing, _ssmvpSource = Nothing,
...
instance FromJSON SSMValidationParameters where
        parseJSON
          = withObject "SSMValidationParameters"
              (\ x ->
                 SSMValidationParameters' <$>
                   (x .:? "instanceId") <*> (x .:? "command") <*>
                     (x .:? "executionTimeoutSeconds")
                     <*> (x .:? "scriptType")
                     <*> (x .:? "source")
                     <*> (x .:? "outputS3BucketName"))

instance Hashable SSMValidationParameters where

instance NFData SSMValidationParameters where
