...
data UserDataValidationParameters = UserDataValidationParameters'{_udvpScriptType
                                                                  ::
                                                                  !(Maybe
                                                                      ScriptType),
                                                                  _udvpSource ::
...
userDataValidationParameters
    :: UserDataValidationParameters
userDataValidationParameters
  = UserDataValidationParameters'{_udvpScriptType =
                                    Nothing,
                                  _udvpSource = Nothing}
...
instance FromJSON UserDataValidationParameters where
        parseJSON
          = withObject "UserDataValidationParameters"
              (\ x ->
                 UserDataValidationParameters' <$>
                   (x .:? "scriptType") <*> (x .:? "source"))

instance Hashable UserDataValidationParameters where

instance NFData UserDataValidationParameters where
