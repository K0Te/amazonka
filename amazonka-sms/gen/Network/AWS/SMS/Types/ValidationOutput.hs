...
data ValidationOutput = ValidationOutput'{_voStatus
...
validationOutput
    :: ValidationOutput
validationOutput
  = ValidationOutput'{_voStatus = Nothing,
                      _voAppValidationOutput = Nothing,
                      _voLatestValidationTime = Nothing, _voName = Nothing,
...
instance FromJSON ValidationOutput where
        parseJSON
          = withObject "ValidationOutput"
              (\ x ->
                 ValidationOutput' <$>
                   (x .:? "status") <*> (x .:? "appValidationOutput")
...
instance Hashable ValidationOutput where

instance NFData ValidationOutput where
