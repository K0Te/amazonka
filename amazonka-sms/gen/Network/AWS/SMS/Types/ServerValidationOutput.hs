...
newtype ServerValidationOutput = ServerValidationOutput'{_svoServer
                                                         :: Maybe Server}
...
instance FromJSON ServerValidationOutput where
        parseJSON
          = withObject "ServerValidationOutput"
              (\ x -> ServerValidationOutput' <$> (x .:? "server"))

instance Hashable ServerValidationOutput where

instance NFData ServerValidationOutput where
