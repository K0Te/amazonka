...
data AppValidationConfiguration = AppValidationConfiguration'{_avcSsmValidationParameters
                                                              ::
                                                              !(Maybe
...
appValidationConfiguration
    :: AppValidationConfiguration
appValidationConfiguration
  = AppValidationConfiguration'{_avcSsmValidationParameters
                                  = Nothing,
                                _avcName = Nothing, _avcValidationId = Nothing,
...
instance FromJSON AppValidationConfiguration where
        parseJSON
          = withObject "AppValidationConfiguration"
              (\ x ->
                 AppValidationConfiguration' <$>
                   (x .:? "ssmValidationParameters") <*> (x .:? "name")
                     <*> (x .:? "validationId")
                     <*> (x .:? "appValidationStrategy"))

instance Hashable AppValidationConfiguration where

instance NFData AppValidationConfiguration where
