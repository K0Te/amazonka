...
data ShardFilter = ShardFilter'{_sfTimestamp ::
                                !(Maybe POSIX),
...
shardFilter
    :: ShardFilterType -- ^ 'sfType'
    -> ShardFilter
shardFilter pType_
  = ShardFilter'{_sfTimestamp = Nothing,
                 _sfShardId = Nothing, _sfType = pType_}
...
instance Hashable ShardFilter where

instance NFData ShardFilter where
