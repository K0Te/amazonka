...
data UngroupResources = UngroupResources'{_urGroup ::
                                          !Text,
...
ungroupResources
    :: Text -- ^ 'urGroup'
    -> NonEmpty Text -- ^ 'urResourceARNs'
    -> UngroupResources
ungroupResources pGroup_ pResourceARNs_
  = UngroupResources'{_urGroup = pGroup_,
...
instance AWSRequest UngroupResources where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 UngroupResourcesResponse' <$>
                   (x .?> "Succeeded") <*> (x .?> "Failed" .!@ mempty)
                     <*> (pure (fromEnum s)))

instance Hashable UngroupResources where

instance NFData UngroupResources where
...
data UngroupResourcesResponse = UngroupResourcesResponse'{_urrsSucceeded
                                                          ::
                                                          !(Maybe (List1 Text)),
                                                          _urrsFailed ::
                                                          !(Maybe
...
ungroupResourcesResponse pResponseStatus_
  = UngroupResourcesResponse'{_urrsSucceeded = Nothing,
                              _urrsFailed = Nothing,
...
instance NFData UngroupResourcesResponse where
