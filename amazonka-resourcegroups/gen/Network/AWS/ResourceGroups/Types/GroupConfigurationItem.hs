...
data GroupConfigurationItem = GroupConfigurationItem'{_gciParameters
                                                      ::
                                                      !(Maybe
...
groupConfigurationItem pType_
  = GroupConfigurationItem'{_gciParameters = Nothing,
                            _gciType = pType_}
...
instance FromJSON GroupConfigurationItem where
        parseJSON
          = withObject "GroupConfigurationItem"
              (\ x ->
                 GroupConfigurationItem' <$>
                   (x .:? "Parameters" .!= mempty) <*> (x .: "Type"))

instance Hashable GroupConfigurationItem where

instance NFData GroupConfigurationItem where
