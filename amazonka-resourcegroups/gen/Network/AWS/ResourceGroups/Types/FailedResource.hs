...
data FailedResource = FailedResource'{_frResourceARN
                                      :: !(Maybe Text),
...
failedResource
    :: FailedResource
failedResource
  = FailedResource'{_frResourceARN = Nothing,
                    _frErrorCode = Nothing, _frErrorMessage = Nothing}
...
instance FromJSON FailedResource where
        parseJSON
          = withObject "FailedResource"
              (\ x ->
                 FailedResource' <$>
                   (x .:? "ResourceArn") <*> (x .:? "ErrorCode") <*>
                     (x .:? "ErrorMessage"))

instance Hashable FailedResource where

instance NFData FailedResource where
