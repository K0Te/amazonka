...
data PostContent = PostContent'{_pcAccept ::
                                !(Maybe Text),
                                ...
                                !(Maybe (Sensitive Text)),
                                _pcBotName :: !Text, _pcBotAlias :: !Text,
                                _pcUserId :: !Text, _pcContentType :: !Text,
...
postContent
    ...
    -> PostContent
postContent pBotName_ pBotAlias_ pUserId_
  pContentType_ pInputStream_
  = PostContent'{_pcAccept = Nothing,
                 _pcActiveContexts = Nothing,
                 _pcRequestAttributes = Nothing,
                 _pcSessionAttributes = Nothing,
                 _pcBotName = pBotName_, _pcBotAlias = pBotAlias_,
...
data PostContentResponse = PostContentResponse'{_pcrsSentimentResponse
                                                ...
                                                _pcrsActiveContexts ::
                                                !(Maybe (Sensitive Text)),
                                                _pcrsAlternativeIntents ::
                                                !(Maybe Text),
                                                _pcrsInputTranscript ::
                                                !(Maybe Text),
                                                _pcrsMessageFormat ::
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'pcrsSentimentResponse' - The sentiment expressed in an utterance. When the bot is configured to send utterances to Amazon Comprehend for sentiment analysis, this field contains the result of the analysis.
--
-- * 'pcrsNluIntentConfidence' - Provides a score that indicates how confident Amazon Lex is that the returned intent is the one that matches the user's intent. The score is between 0.0 and 1.0. The score is a relative score, not an absolute score. The score may change based on improvements to Amazon Lex. 
--
...
--
-- * 'pcrsBotVersion' - The version of the bot that responded to the conversation. You can use this information to help determine if one version of a bot is performing better than another version.
--
...
--
-- * 'pcrsActiveContexts' - A list of active contexts for the session. A context can be set when an intent is fulfilled or by calling the @PostContent@ , @PostText@ , or @PutSession@ operation. You can use a context to control the intents that can follow up an intent, or to modify the operation of your application.
--
-- * 'pcrsAlternativeIntents' - One to four alternative intents that may be applicable to the user's intent. Each alternative includes a score that indicates how confident Amazon Lex is that the intent matches the user's intent. The intents are sorted by the confidence score.
--
...
postContentResponse pResponseStatus_ pAudioStream_
  = PostContentResponse'{_pcrsSentimentResponse =
                           Nothing,
                         _pcrsNluIntentConfidence = Nothing,
                         _pcrsSlots = Nothing, _pcrsIntentName = Nothing,
                         _pcrsBotVersion = Nothing,
                         _pcrsDialogState = Nothing,
                         _pcrsActiveContexts = Nothing,
                         _pcrsAlternativeIntents = Nothing,
...
pcrsNluIntentConfidence :: Lens' PostContentResponse (Maybe Text)
pcrsNluIntentConfidence = lens _pcrsNluIntentConfidence (\ s a -> s{_pcrsNluIntentConfidence = a})
...
pcrsBotVersion :: Lens' PostContentResponse (Maybe Text)
pcrsBotVersion = lens _pcrsBotVersion (\ s a -> s{_pcrsBotVersion = a})
...
pcrsActiveContexts :: Lens' PostContentResponse (Maybe Text)
pcrsActiveContexts = lens _pcrsActiveContexts (\ s a -> s{_pcrsActiveContexts = a}) . mapping _Sensitive
...
pcrsAlternativeIntents :: Lens' PostContentResponse (Maybe Text)
pcrsAlternativeIntents = lens _pcrsAlternativeIntents (\ s a -> s{_pcrsAlternativeIntents = a})
