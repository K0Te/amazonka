...
newtype IntentConfidence = IntentConfidence'{_icScore
                                             :: Maybe Double}
...
instance FromJSON IntentConfidence where
        parseJSON
          = withObject "IntentConfidence"
              (\ x -> IntentConfidence' <$> (x .:? "score"))

instance Hashable IntentConfidence where

instance NFData IntentConfidence where
