...
data ActiveContext = ActiveContext'{_acName :: !Text,
...
activeContext
    :: Text -- ^ 'acName'
    -> ActiveContextTimeToLive -- ^ 'acTimeToLive'
    -> ActiveContext
activeContext pName_ pTimeToLive_
  = ActiveContext'{_acName = pName_,
                   _acTimeToLive = pTimeToLive_, _acParameters = mempty}
...
instance FromJSON ActiveContext where
        parseJSON
          = withObject "ActiveContext"
              (\ x ->
                 ActiveContext' <$>
                   (x .: "name") <*> (x .: "timeToLive") <*>
                     (x .:? "parameters" .!= mempty))

instance Hashable ActiveContext where

instance NFData ActiveContext where
