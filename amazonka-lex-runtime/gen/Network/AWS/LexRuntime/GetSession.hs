...
data GetSession = GetSession'{_gsCheckpointLabelFilter
                              :: !(Maybe Text),
                              _gsBotName :: !Text, _gsBotAlias :: !Text,
                              _gsUserId :: !Text}
...
getSession pBotName_ pBotAlias_ pUserId_
  = GetSession'{_gsCheckpointLabelFilter = Nothing,
                _gsBotName = pBotName_, _gsBotAlias = pBotAlias_,
                _gsUserId = pUserId_}
...
instance Hashable GetSession where

instance NFData GetSession where
...
instance ToPath GetSession where
        toPath GetSession'{..}
          = mconcat
              ["/bot/", toBS _gsBotName, "/alias/",
               toBS _gsBotAlias, "/user/", toBS _gsUserId,
               "/session/"]
...
data GetSessionResponse = GetSessionResponse'{_gsrsActiveContexts
                                              ::
                                              !(Maybe
...
getSessionResponse pResponseStatus_
  = GetSessionResponse'{_gsrsActiveContexts = Nothing,
...
instance NFData GetSessionResponse where
