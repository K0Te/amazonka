...
data DeleteSession = DeleteSession'{_dsBotName ::
                                    !Text,
                                    _dsBotAlias :: !Text, _dsUserId :: !Text}
...
deleteSession pBotName_ pBotAlias_ pUserId_
  = DeleteSession'{_dsBotName = pBotName_,
                   _dsBotAlias = pBotAlias_, _dsUserId = pUserId_}
...
instance Hashable DeleteSession where

instance NFData DeleteSession where
...
instance ToPath DeleteSession where
        toPath DeleteSession'{..}
          = mconcat
              ["/bot/", toBS _dsBotName, "/alias/",
               toBS _dsBotAlias, "/user/", toBS _dsUserId,
               "/session"]
...
data DeleteSessionResponse = DeleteSessionResponse'{_dsrsBotAlias
                                                    :: !(Maybe Text),
                                                    _dsrsBotName ::
                                                    !(Maybe Text),
                                                    _dsrsUserId ::
...
deleteSessionResponse pResponseStatus_
  = DeleteSessionResponse'{_dsrsBotAlias = Nothing,
                           _dsrsBotName = Nothing, _dsrsUserId = Nothing,
...
instance NFData DeleteSessionResponse where
