...
data LaunchPath = LaunchPath'{_lpName ::
                              !(Maybe Text),
                              _lpId :: !(Maybe Text)}
...
launchPath
    :: LaunchPath
launchPath
  = LaunchPath'{_lpName = Nothing, _lpId = Nothing}
...
instance FromJSON LaunchPath where
        parseJSON
          = withObject "LaunchPath"
              (\ x ->
                 LaunchPath' <$> (x .:? "Name") <*> (x .:? "Id"))

instance Hashable LaunchPath where

instance NFData LaunchPath where
