...
newtype SchemaDefinition = SchemaDefinition'{_sdColumns
                                             :: Maybe [Column]}
...
instance FromJSON SchemaDefinition where
        parseJSON
          = withObject "SchemaDefinition"
              (\ x ->
                 SchemaDefinition' <$> (x .:? "columns" .!= mempty))

instance Hashable SchemaDefinition where

instance NFData SchemaDefinition where
