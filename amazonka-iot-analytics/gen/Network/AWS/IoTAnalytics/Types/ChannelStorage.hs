...
data ChannelStorage = ChannelStorage'{_csServiceManagedS3
                                      ::
...
channelStorage
    :: ChannelStorage
channelStorage
  = ChannelStorage'{_csServiceManagedS3 = Nothing,
...
instance FromJSON ChannelStorage where
        parseJSON
          = withObject "ChannelStorage"
              (\ x ->
                 ChannelStorage' <$>
                   (x .:? "serviceManagedS3") <*>
                     (x .:? "customerManagedS3"))

instance Hashable ChannelStorage where

instance NFData ChannelStorage where
