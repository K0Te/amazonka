...
data DatastoreStorageSummary = DatastoreStorageSummary'{_dssServiceManagedS3
                                                        ::
                                                        !(Maybe
...
datastoreStorageSummary
    :: DatastoreStorageSummary
datastoreStorageSummary
  = DatastoreStorageSummary'{_dssServiceManagedS3 =
                               Nothing,
...
instance FromJSON DatastoreStorageSummary where
        parseJSON
          = withObject "DatastoreStorageSummary"
              (\ x ->
                 DatastoreStorageSummary' <$>
                   (x .:? "serviceManagedS3") <*>
                     (x .:? "customerManagedS3"))

instance Hashable DatastoreStorageSummary where

instance NFData DatastoreStorageSummary where
