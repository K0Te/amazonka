...
newtype ParquetConfiguration = ParquetConfiguration'{_pcSchemaDefinition
...
parquetConfiguration
    :: ParquetConfiguration
parquetConfiguration
  = ParquetConfiguration'{_pcSchemaDefinition =
                            Nothing}
...
instance FromJSON ParquetConfiguration where
        parseJSON
          = withObject "ParquetConfiguration"
              (\ x ->
                 ParquetConfiguration' <$> (x .:? "schemaDefinition"))

instance Hashable ParquetConfiguration where

instance NFData ParquetConfiguration where
