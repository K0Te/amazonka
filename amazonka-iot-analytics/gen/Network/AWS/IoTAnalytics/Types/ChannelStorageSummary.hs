...
data ChannelStorageSummary = ChannelStorageSummary'{_cssServiceManagedS3
                                                    ::
                                                    !(Maybe
...
channelStorageSummary
    :: ChannelStorageSummary
channelStorageSummary
  = ChannelStorageSummary'{_cssServiceManagedS3 =
                             Nothing,
...
instance FromJSON ChannelStorageSummary where
        parseJSON
          = withObject "ChannelStorageSummary"
              (\ x ->
                 ChannelStorageSummary' <$>
                   (x .:? "serviceManagedS3") <*>
                     (x .:? "customerManagedS3"))

instance Hashable ChannelStorageSummary where

instance NFData ChannelStorageSummary where
