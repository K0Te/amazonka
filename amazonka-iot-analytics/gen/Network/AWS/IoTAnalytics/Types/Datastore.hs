...
--
-- * 'datFileFormatConfiguration' - Contains the configuration information of file formats. AWS IoT Analytics data stores support JSON and <https://parquet.apache.org/ Parquet> . The default file format is JSON. You can specify only one format. You can't change the file format after you create the data store.
--
...
datastore
  = Datastore'{_datCreationTime = Nothing,
               ...
               _datArn = Nothing,
               _datFileFormatConfiguration = Nothing,
...
datFileFormatConfiguration :: Lens' Datastore (Maybe FileFormatConfiguration)
datFileFormatConfiguration = lens _datFileFormatConfiguration (\ s a -> s{_datFileFormatConfiguration = a})
