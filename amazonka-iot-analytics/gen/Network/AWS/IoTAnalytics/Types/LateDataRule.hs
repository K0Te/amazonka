...
data LateDataRule = LateDataRule'{_ldrRuleName ::
                                  !(Maybe Text),
...
lateDataRule
    :: LateDataRuleConfiguration -- ^ 'ldrRuleConfiguration'
    -> LateDataRule
lateDataRule pRuleConfiguration_
  = LateDataRule'{_ldrRuleName = Nothing,
...
instance FromJSON LateDataRule where
        parseJSON
          = withObject "LateDataRule"
              (\ x ->
                 LateDataRule' <$>
                   (x .:? "ruleName") <*> (x .: "ruleConfiguration"))

instance Hashable LateDataRule where

instance NFData LateDataRule where
