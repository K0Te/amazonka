...
newtype ResumeCluster = ResumeCluster'{_resClusterIdentifier
                                       :: Text}
...
resumeCluster
    :: Text -- ^ 'resClusterIdentifier'
    -> ResumeCluster
resumeCluster pClusterIdentifier_
  = ResumeCluster'{_resClusterIdentifier =
                     pClusterIdentifier_}
...
instance AWSRequest ResumeCluster where
        ...
        response
          = receiveXMLWrapper "ResumeClusterResult"
              (\ s h x ->
                 ResumeClusterResponse' <$>
                   (x .@? "Cluster") <*> (pure (fromEnum s)))

instance Hashable ResumeCluster where

instance NFData ResumeCluster where
...
data ResumeClusterResponse = ResumeClusterResponse'{_rcrsCluster
                                                    :: !(Maybe Cluster),
...
resumeClusterResponse pResponseStatus_
  = ResumeClusterResponse'{_rcrsCluster = Nothing,
...
instance NFData ResumeClusterResponse where
