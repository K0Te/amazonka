...
newtype PauseCluster = PauseCluster'{_pcClusterIdentifier
                                     :: Text}
...
pauseCluster
    :: Text -- ^ 'pcClusterIdentifier'
    -> PauseCluster
pauseCluster pClusterIdentifier_
  = PauseCluster'{_pcClusterIdentifier =
                    pClusterIdentifier_}
...
instance AWSRequest PauseCluster where
        ...
        response
          = receiveXMLWrapper "PauseClusterResult"
              (\ s h x ->
                 PauseClusterResponse' <$>
                   (x .@? "Cluster") <*> (pure (fromEnum s)))

instance Hashable PauseCluster where

instance NFData PauseCluster where
...
data PauseClusterResponse = PauseClusterResponse'{_pcrsCluster
                                                  :: !(Maybe Cluster),
...
pauseClusterResponse pResponseStatus_
  = PauseClusterResponse'{_pcrsCluster = Nothing,
...
instance NFData PauseClusterResponse where
