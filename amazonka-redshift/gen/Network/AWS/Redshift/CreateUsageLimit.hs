...
data CreateUsageLimit = CreateUsageLimit'{_culPeriod
...
createUsageLimit
    :: Text -- ^ 'culClusterIdentifier'
    -> UsageLimitFeatureType -- ^ 'culFeatureType'
    -> UsageLimitLimitType -- ^ 'culLimitType'
    -> Integer -- ^ 'culAmount'
    -> CreateUsageLimit
createUsageLimit pClusterIdentifier_ pFeatureType_
  pLimitType_ pAmount_
  = CreateUsageLimit'{_culPeriod = Nothing,
                      _culBreachAction = Nothing, _culTags = Nothing,
                      _culClusterIdentifier = pClusterIdentifier_,
                      _culFeatureType = pFeatureType_,
                      _culLimitType = pLimitType_, _culAmount = pAmount_}
...
instance AWSRequest CreateUsageLimit where
        ...
        response
          = receiveXMLWrapper "CreateUsageLimitResult"
              (\ s h x -> parseXML x)

instance Hashable CreateUsageLimit where

instance NFData CreateUsageLimit where
        ...
        toQuery CreateUsageLimit'{..}
          = mconcat
              ["Action" =: ("CreateUsageLimit" :: ByteString),
               ...
               "LimitType" =: _culLimitType, "Amount" =: _culAmount]
