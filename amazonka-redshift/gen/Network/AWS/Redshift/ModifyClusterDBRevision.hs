...
data ModifyClusterDBRevision = ModifyClusterDBRevision'{_mcdrClusterIdentifier
                                                        :: !Text,
                                                        _mcdrRevisionTarget ::
                                                        !Text}
...
modifyClusterDBRevision
    :: Text -- ^ 'mcdrClusterIdentifier'
    -> Text -- ^ 'mcdrRevisionTarget'
    -> ModifyClusterDBRevision
modifyClusterDBRevision pClusterIdentifier_
  pRevisionTarget_
  = ModifyClusterDBRevision'{_mcdrClusterIdentifier =
                               pClusterIdentifier_,
...
instance AWSRequest ModifyClusterDBRevision where
        type Rs ModifyClusterDBRevision =
             ModifyClusterDBRevisionResponse
        request = postQuery redshift
        response
          = receiveXMLWrapper "ModifyClusterDbRevisionResult"
              (\ s h x ->
                 ModifyClusterDBRevisionResponse' <$>
                   (x .@? "Cluster") <*> (pure (fromEnum s)))

instance Hashable ModifyClusterDBRevision where

instance NFData ModifyClusterDBRevision where
...
data ModifyClusterDBRevisionResponse = ModifyClusterDBRevisionResponse'{_mcdrrsCluster
                                                                        ::
                                                                        !(Maybe
                                                                            Cluster),
...
modifyClusterDBRevisionResponse
    :: Int -- ^ 'mcdrrsResponseStatus'
    -> ModifyClusterDBRevisionResponse
modifyClusterDBRevisionResponse pResponseStatus_
  = ModifyClusterDBRevisionResponse'{_mcdrrsCluster =
                                       Nothing,
...
instance NFData ModifyClusterDBRevisionResponse where
