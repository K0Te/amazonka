...
data GetReservedNodeExchangeOfferings = GetReservedNodeExchangeOfferings'{_grneoMarker
                                                                          ::
                                                                          !(Maybe
                                                                              Text),
                                                                          _grneoMaxRecords
...
getReservedNodeExchangeOfferings
    :: Text -- ^ 'grneoReservedNodeId'
    -> GetReservedNodeExchangeOfferings
getReservedNodeExchangeOfferings pReservedNodeId_
  = GetReservedNodeExchangeOfferings'{_grneoMarker =
                                        Nothing,
...
instance AWSPager GetReservedNodeExchangeOfferings
         where
        page rq rs
          | stop (rs ^. grneorsMarker) = Nothing
          | stop (rs ^. grneorsReservedNodeOfferings) = Nothing
          | otherwise =
            Just $ rq & grneoMarker .~ rs ^. grneorsMarker

instance AWSRequest GetReservedNodeExchangeOfferings
         where
...
instance ToHeaders GetReservedNodeExchangeOfferings
         where
        toHeaders = const mempty

instance ToPath GetReservedNodeExchangeOfferings
         where
        toPath = const "/"

instance ToQuery GetReservedNodeExchangeOfferings
         where
...
data GetReservedNodeExchangeOfferingsResponse = GetReservedNodeExchangeOfferingsResponse'{_grneorsReservedNodeOfferings
                                                                                          ::
                                                                                          !(Maybe
...
getReservedNodeExchangeOfferingsResponse
    :: Int -- ^ 'grneorsResponseStatus'
    -> GetReservedNodeExchangeOfferingsResponse
getReservedNodeExchangeOfferingsResponse
  pResponseStatus_
  = GetReservedNodeExchangeOfferingsResponse'{_grneorsReservedNodeOfferings
                                                = Nothing,
