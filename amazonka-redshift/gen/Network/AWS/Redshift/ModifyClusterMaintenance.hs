...
data ModifyClusterMaintenance = ModifyClusterMaintenance'{_mcmDeferMaintenanceEndTime
                                                          :: !(Maybe ISO8601),
...
modifyClusterMaintenance
    :: Text -- ^ 'mcmClusterIdentifier'
    -> ModifyClusterMaintenance
modifyClusterMaintenance pClusterIdentifier_
  = ModifyClusterMaintenance'{_mcmDeferMaintenanceEndTime
                                = Nothing,
...
instance AWSRequest ModifyClusterMaintenance where
        type Rs ModifyClusterMaintenance =
             ModifyClusterMaintenanceResponse
        request = postQuery redshift
        response
          = receiveXMLWrapper "ModifyClusterMaintenanceResult"
              (\ s h x ->
                 ModifyClusterMaintenanceResponse' <$>
                   (x .@? "Cluster") <*> (pure (fromEnum s)))

instance Hashable ModifyClusterMaintenance where

instance NFData ModifyClusterMaintenance where
...
data ModifyClusterMaintenanceResponse = ModifyClusterMaintenanceResponse'{_mcmrsCluster
                                                                          ::
                                                                          !(Maybe
                                                                              Cluster),
...
modifyClusterMaintenanceResponse
    :: Int -- ^ 'mcmrsResponseStatus'
    -> ModifyClusterMaintenanceResponse
modifyClusterMaintenanceResponse pResponseStatus_
  = ModifyClusterMaintenanceResponse'{_mcmrsCluster =
                                        Nothing,
