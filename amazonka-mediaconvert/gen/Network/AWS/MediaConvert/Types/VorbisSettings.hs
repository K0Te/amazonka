...
data VorbisSettings = VorbisSettings'{_vsChannels ::
                                      !(Maybe Nat),
...
vorbisSettings
    :: VorbisSettings
vorbisSettings
  = VorbisSettings'{_vsChannels = Nothing,
                    _vsSampleRate = Nothing, _vsVbrQuality = Nothing}
...
instance FromJSON VorbisSettings where
        parseJSON
          = withObject "VorbisSettings"
              (\ x ->
                 VorbisSettings' <$>
                   (x .:? "channels") <*> (x .:? "sampleRate") <*>
                     (x .:? "vbrQuality"))

instance Hashable VorbisSettings where

instance NFData VorbisSettings where
