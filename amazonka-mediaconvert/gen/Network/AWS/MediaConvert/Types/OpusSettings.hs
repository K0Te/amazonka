...
data OpusSettings = OpusSettings'{_osChannels ::
                                  !(Maybe Nat),
...
opusSettings
    :: OpusSettings
opusSettings
  = OpusSettings'{_osChannels = Nothing,
                  _osSampleRate = Nothing, _osBitrate = Nothing}
...
instance FromJSON OpusSettings where
        parseJSON
          = withObject "OpusSettings"
              (\ x ->
                 OpusSettings' <$>
                   (x .:? "channels") <*> (x .:? "sampleRate") <*>
                     (x .:? "bitrate"))

instance Hashable OpusSettings where

instance NFData OpusSettings where
