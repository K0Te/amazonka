...
data ReplicationDestination = ReplicationDestination'{_rdRegion
                                                      :: !Text,
                                                      _rdRegistryId :: !Text}
...
replicationDestination pRegion_ pRegistryId_
  = ReplicationDestination'{_rdRegion = pRegion_,
...
instance FromJSON ReplicationDestination where
        parseJSON
          = withObject "ReplicationDestination"
              (\ x ->
                 ReplicationDestination' <$>
                   (x .: "region") <*> (x .: "registryId"))

instance Hashable ReplicationDestination where

instance NFData ReplicationDestination where
