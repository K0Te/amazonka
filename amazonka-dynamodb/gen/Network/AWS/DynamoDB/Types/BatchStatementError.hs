...
data BatchStatementError = BatchStatementError'{_bseCode
                                                ::
                                                !(Maybe
...
batchStatementError
    :: BatchStatementError
batchStatementError
  = BatchStatementError'{_bseCode = Nothing,
                         _bseMessage = Nothing}
...
instance FromJSON BatchStatementError where
        parseJSON
          = withObject "BatchStatementError"
              (\ x ->
                 BatchStatementError' <$>
                   (x .:? "Code") <*> (x .:? "Message"))

instance Hashable BatchStatementError where

instance NFData BatchStatementError where
