...
data ExportSummary = ExportSummary'{_esExportStatus
                                    :: !(Maybe ExportStatus),
...
exportSummary
    :: ExportSummary
exportSummary
  = ExportSummary'{_esExportStatus = Nothing,
...
instance FromJSON ExportSummary where
        parseJSON
          = withObject "ExportSummary"
              (\ x ->
                 ExportSummary' <$>
                   (x .:? "ExportStatus") <*> (x .:? "ExportArn"))

instance Hashable ExportSummary where

instance NFData ExportSummary where
