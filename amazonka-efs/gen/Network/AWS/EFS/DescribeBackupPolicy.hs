...
newtype DescribeBackupPolicy = DescribeBackupPolicy'{_dbpFileSystemId
                                                     :: Text}
...
describeBackupPolicy
    :: Text -- ^ 'dbpFileSystemId'
    -> DescribeBackupPolicy
describeBackupPolicy pFileSystemId_
  = DescribeBackupPolicy'{_dbpFileSystemId =
                            pFileSystemId_}
...
instance AWSRequest DescribeBackupPolicy where
        type Rs DescribeBackupPolicy =
             BackupPolicyDescription
        request = get efs
        response = receiveJSON (\ s h x -> eitherParseJSON x)

instance Hashable DescribeBackupPolicy where

instance NFData DescribeBackupPolicy where
...
instance ToPath DescribeBackupPolicy where
        toPath DescribeBackupPolicy'{..}
          = mconcat
              ["/2015-02-01/file-systems/", toBS _dbpFileSystemId,
               "/backup-policy"]
