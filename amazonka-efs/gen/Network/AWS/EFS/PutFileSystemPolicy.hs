...
data PutFileSystemPolicy = PutFileSystemPolicy'{_pfspBypassPolicyLockoutSafetyCheck
                                                :: !(Maybe Bool),
...
putFileSystemPolicy
    :: Text -- ^ 'pfspFileSystemId'
    -> Text -- ^ 'pfspPolicy'
    -> PutFileSystemPolicy
putFileSystemPolicy pFileSystemId_ pPolicy_
  = PutFileSystemPolicy'{_pfspBypassPolicyLockoutSafetyCheck
                           = Nothing,
...
instance AWSRequest PutFileSystemPolicy where
        type Rs PutFileSystemPolicy =
             FileSystemPolicyDescription
        request = putJSON efs
        response = receiveJSON (\ s h x -> eitherParseJSON x)

instance Hashable PutFileSystemPolicy where

instance NFData PutFileSystemPolicy where
...
instance ToPath PutFileSystemPolicy where
        toPath PutFileSystemPolicy'{..}
          = mconcat
              ["/2015-02-01/file-systems/", toBS _pfspFileSystemId,
               "/policy"]
