...
data PutBackupPolicy = PutBackupPolicy'{_pbpFileSystemId
                                        :: !Text,
...
putBackupPolicy
    :: Text -- ^ 'pbpFileSystemId'
    -> BackupPolicy -- ^ 'pbpBackupPolicy'
    -> PutBackupPolicy
putBackupPolicy pFileSystemId_ pBackupPolicy_
  = PutBackupPolicy'{_pbpFileSystemId = pFileSystemId_,
...
instance Hashable PutBackupPolicy where

instance NFData PutBackupPolicy where
...
instance ToPath PutBackupPolicy where
        toPath PutBackupPolicy'{..}
          = mconcat
              ["/2015-02-01/file-systems/", toBS _pbpFileSystemId,
               "/backup-policy"]
