...
data ConnectionAlias = ConnectionAlias'{_caState ::
...
connectionAlias
    :: ConnectionAlias
connectionAlias
  = ConnectionAlias'{_caState = Nothing,
                     _caOwnerAccountId = Nothing, _caAliasId = Nothing,
...
instance FromJSON ConnectionAlias where
        parseJSON
          = withObject "ConnectionAlias"
              (\ x ->
                 ConnectionAlias' <$>
                   (x .:? "State") <*> (x .:? "OwnerAccountId") <*>
                     (x .:? "AliasId")
                     <*> (x .:? "Associations")
                     <*> (x .:? "ConnectionString"))

instance Hashable ConnectionAlias where

instance NFData ConnectionAlias where
