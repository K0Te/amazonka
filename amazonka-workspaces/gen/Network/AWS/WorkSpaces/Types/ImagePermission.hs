...
newtype ImagePermission = ImagePermission'{_ipSharedAccountId
                                           :: Maybe Text}
...
instance FromJSON ImagePermission where
        parseJSON
          = withObject "ImagePermission"
              (\ x ->
                 ImagePermission' <$> (x .:? "SharedAccountId"))

instance Hashable ImagePermission where

instance NFData ImagePermission where
