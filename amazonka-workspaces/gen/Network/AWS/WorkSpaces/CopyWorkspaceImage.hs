...
data CopyWorkspaceImage = CopyWorkspaceImage'{_cwiDescription
                                              :: !(Maybe Text),
                                              _cwiTags :: !(Maybe [Tag]),
                                              _cwiName :: !Text,
...
copyWorkspaceImage
    :: Text -- ^ 'cwiName'
    -> Text -- ^ 'cwiSourceImageId'
    -> Text -- ^ 'cwiSourceRegion'
    -> CopyWorkspaceImage
copyWorkspaceImage pName_ pSourceImageId_
  pSourceRegion_
  = CopyWorkspaceImage'{_cwiDescription = Nothing,
                        _cwiTags = Nothing, _cwiName = pName_,
...
instance Hashable CopyWorkspaceImage where

instance NFData CopyWorkspaceImage where
          ...
          = object
              (catMaybes
                 [("Description" .=) <$> _cwiDescription,
                  ("Tags" .=) <$> _cwiTags, Just ("Name" .= _cwiName),
...
data CopyWorkspaceImageResponse = CopyWorkspaceImageResponse'{_cwirsImageId
                                                              :: !(Maybe Text),
                                                              _cwirsResponseStatus
                                                              :: !Int}
...
copyWorkspaceImageResponse
    :: Int -- ^ 'cwirsResponseStatus'
    -> CopyWorkspaceImageResponse
copyWorkspaceImageResponse pResponseStatus_
  = CopyWorkspaceImageResponse'{_cwirsImageId =
                                  Nothing,
...
instance NFData CopyWorkspaceImageResponse where
