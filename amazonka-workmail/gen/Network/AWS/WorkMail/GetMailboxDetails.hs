...
data GetMailboxDetails = GetMailboxDetails'{_gmdOrganizationId
                                            :: !Text,
                                            _gmdUserId :: !Text}
...
getMailboxDetails
    :: Text -- ^ 'gmdOrganizationId'
    -> Text -- ^ 'gmdUserId'
    -> GetMailboxDetails
getMailboxDetails pOrganizationId_ pUserId_
  = GetMailboxDetails'{_gmdOrganizationId =
                         pOrganizationId_,
                       _gmdUserId = pUserId_}
...
instance AWSRequest GetMailboxDetails where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 GetMailboxDetailsResponse' <$>
                   (x .?> "MailboxQuota") <*> (x .?> "MailboxSize") <*>
                     (pure (fromEnum s)))

instance Hashable GetMailboxDetails where

instance NFData GetMailboxDetails where
...
data GetMailboxDetailsResponse = GetMailboxDetailsResponse'{_gmdrsMailboxQuota
                                                            :: !(Maybe Nat),
                                                            _gmdrsMailboxSize ::
                                                            !(Maybe Double),
...
getMailboxDetailsResponse pResponseStatus_
  = GetMailboxDetailsResponse'{_gmdrsMailboxQuota =
                                 Nothing,
...
instance NFData GetMailboxDetailsResponse where
