...
data FolderConfiguration = FolderConfiguration'{_fcPeriod
                                                :: !(Maybe Nat),
                                                _fcName :: !FolderName,
...
folderConfiguration
    :: FolderName -- ^ 'fcName'
    -> RetentionAction -- ^ 'fcAction'
    -> FolderConfiguration
folderConfiguration pName_ pAction_
  = FolderConfiguration'{_fcPeriod = Nothing,
                         _fcName = pName_, _fcAction = pAction_}
...
instance FromJSON FolderConfiguration where
        parseJSON
          = withObject "FolderConfiguration"
              (\ x ->
                 FolderConfiguration' <$>
                   (x .:? "Period") <*> (x .: "Name") <*>
                     (x .: "Action"))

instance Hashable FolderConfiguration where

instance NFData FolderConfiguration where
