...
data StartMailboxExportJob = StartMailboxExportJob'{_smejDescription
                                                    :: !(Maybe Text),
...
startMailboxExportJob
    :: Text -- ^ 'smejClientToken'
    -> Text -- ^ 'smejOrganizationId'
    -> Text -- ^ 'smejEntityId'
    -> Text -- ^ 'smejRoleARN'
    -> Text -- ^ 'smejKMSKeyARN'
    -> Text -- ^ 'smejS3BucketName'
    -> Text -- ^ 'smejS3Prefix'
    -> StartMailboxExportJob
startMailboxExportJob pClientToken_ pOrganizationId_
  pEntityId_ pRoleARN_ pKMSKeyARN_ pS3BucketName_
  pS3Prefix_
  = StartMailboxExportJob'{_smejDescription = Nothing,
                           _smejClientToken = pClientToken_,
                           _smejOrganizationId = pOrganizationId_,
                           _smejEntityId = pEntityId_, _smejRoleARN = pRoleARN_,
...
instance Hashable StartMailboxExportJob where

instance NFData StartMailboxExportJob where
...
data StartMailboxExportJobResponse = StartMailboxExportJobResponse'{_smejrsJobId
                                                                    ::
                                                                    !(Maybe
                                                                        Text),
                                                                    _smejrsResponseStatus
                                                                    :: !Int}
                                       deriving (Eq, Read, Show, Data, Typeable,
                                                 Generic)
...
startMailboxExportJobResponse
    :: Int -- ^ 'smejrsResponseStatus'
    -> StartMailboxExportJobResponse
startMailboxExportJobResponse pResponseStatus_
  = StartMailboxExportJobResponse'{_smejrsJobId =
                                     Nothing,
...
instance NFData StartMailboxExportJobResponse where
