...
data HostedZoneSummary = HostedZoneSummary'{_hzsHostedZoneId
                                            :: !ResourceId,
                                            _hzsName :: !Text,
...
hostedZoneSummary
    :: ResourceId -- ^ 'hzsHostedZoneId'
    -> Text -- ^ 'hzsName'
    -> HostedZoneOwner -- ^ 'hzsOwner'
    -> HostedZoneSummary
hostedZoneSummary pHostedZoneId_ pName_ pOwner_
  = HostedZoneSummary'{_hzsHostedZoneId =
                         pHostedZoneId_,
                       _hzsName = pName_, _hzsOwner = pOwner_}
...
instance FromXML HostedZoneSummary where
        parseXML x
          = HostedZoneSummary' <$>
              (x .@ "HostedZoneId") <*> (x .@ "Name") <*>
                (x .@ "Owner")

instance Hashable HostedZoneSummary where

instance NFData HostedZoneSummary where
