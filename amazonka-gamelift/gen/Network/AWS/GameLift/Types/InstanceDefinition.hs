...
data InstanceDefinition = InstanceDefinition'{_idWeightedCapacity
                                              :: !(Maybe Text),
                                              _idInstanceType ::
...
instanceDefinition
    :: GameServerGroupInstanceType -- ^ 'idInstanceType'
    -> InstanceDefinition
instanceDefinition pInstanceType_
  = InstanceDefinition'{_idWeightedCapacity = Nothing,
...
instance FromJSON InstanceDefinition where
        parseJSON
          = withObject "InstanceDefinition"
              (\ x ->
                 InstanceDefinition' <$>
                   (x .:? "WeightedCapacity") <*> (x .: "InstanceType"))

instance Hashable InstanceDefinition where

instance NFData InstanceDefinition where
