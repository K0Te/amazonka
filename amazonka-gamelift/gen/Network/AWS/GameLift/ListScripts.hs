...
data ListScripts = ListScripts'{_lsNextToken ::
                                !(Maybe Text),
                                _lsLimit :: !(Maybe Nat)}
...
listScripts
    :: ListScripts
listScripts
  = ListScripts'{_lsNextToken = Nothing,
                 _lsLimit = Nothing}
...
instance AWSRequest ListScripts where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 ListScriptsResponse' <$>
                   (x .?> "Scripts" .!@ mempty) <*> (x .?> "NextToken")
                     <*> (pure (fromEnum s)))

instance Hashable ListScripts where

instance NFData ListScripts where
...
data ListScriptsResponse = ListScriptsResponse'{_lsrsScripts
                                                :: !(Maybe [Script]),
...
listScriptsResponse pResponseStatus_
  = ListScriptsResponse'{_lsrsScripts = Nothing,
...
instance NFData ListScriptsResponse where
