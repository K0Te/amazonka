...
data ModifyUserGroup = ModifyUserGroup'{_mugUserIdsToAdd
                                        :: !(Maybe (List1 Text)),
                                        _mugUserIdsToRemove ::
                                        !(Maybe (List1 Text)),
...
modifyUserGroup pUserGroupId_
  = ModifyUserGroup'{_mugUserIdsToAdd = Nothing,
...
instance AWSRequest ModifyUserGroup where
        ...
        response
          = receiveXMLWrapper "ModifyUserGroupResult"
              (\ s h x -> parseXML x)

instance Hashable ModifyUserGroup where

instance NFData ModifyUserGroup where
