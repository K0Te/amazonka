...
data CreateUserGroup = CreateUserGroup'{_cugUserIds
                                        :: !(Maybe (List1 Text)),
...
createUserGroup pUserGroupId_ pEngine_
  = CreateUserGroup'{_cugUserIds = Nothing,
...
instance AWSRequest CreateUserGroup where
        ...
        response
          = receiveXMLWrapper "CreateUserGroupResult"
              (\ s h x -> parseXML x)

instance Hashable CreateUserGroup where

instance NFData CreateUserGroup where
