...
data DescribeUsers = DescribeUsers'{_duFilters ::
                                    !(Maybe [Filter]),
...
describeUsers
    :: DescribeUsers
describeUsers
  = DescribeUsers'{_duFilters = Nothing,
                   _duEngine = Nothing, _duUserId = Nothing,
                   _duMarker = Nothing, _duMaxRecords = Nothing}
...
instance AWSRequest DescribeUsers where
        ...
        response
          = receiveXMLWrapper "DescribeUsersResult"
              (\ s h x ->
                 DescribeUsersResponse' <$>
                   (x .@? "Users" .!@ mempty >>=
        ...
        toQuery DescribeUsers'{..}
          = mconcat
              ["Action" =: ("DescribeUsers" :: ByteString),
               ...
               "Engine" =: _duEngine, "UserId" =: _duUserId,
               "Marker" =: _duMarker, "MaxRecords" =: _duMaxRecords]
