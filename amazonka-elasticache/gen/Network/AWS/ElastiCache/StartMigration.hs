...
data StartMigration = StartMigration'{_smReplicationGroupId
                                      :: !Text,
...
startMigration
    :: Text -- ^ 'smReplicationGroupId'
    -> StartMigration
startMigration pReplicationGroupId_
  = StartMigration'{_smReplicationGroupId =
...
instance AWSRequest StartMigration where
        ...
        response
          = receiveXMLWrapper "StartMigrationResult"
              (\ s h x ->
                 StartMigrationResponse' <$>
                   (x .@? "ReplicationGroup") <*> (pure (fromEnum s)))

instance Hashable StartMigration where

instance NFData StartMigration where
...
data StartMigrationResponse = StartMigrationResponse'{_smrsReplicationGroup
                                                      ::
...
startMigrationResponse pResponseStatus_
  = StartMigrationResponse'{_smrsReplicationGroup =
                              Nothing,
...
instance NFData StartMigrationResponse where
