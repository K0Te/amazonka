...
data CompleteMigration = CompleteMigration'{_cmForce
                                            :: !(Maybe Bool),
...
completeMigration
    :: Text -- ^ 'cmReplicationGroupId'
    -> CompleteMigration
completeMigration pReplicationGroupId_
  = CompleteMigration'{_cmForce = Nothing,
...
instance AWSRequest CompleteMigration where
        ...
        response
          = receiveXMLWrapper "CompleteMigrationResult"
              (\ s h x ->
                 CompleteMigrationResponse' <$>
                   (x .@? "ReplicationGroup") <*> (pure (fromEnum s)))

instance Hashable CompleteMigration where

instance NFData CompleteMigration where
...
data CompleteMigrationResponse = CompleteMigrationResponse'{_cmrsReplicationGroup
                                                            ::
                                                            !(Maybe
                                                                ReplicationGroup),
                                                            _cmrsResponseStatus
...
completeMigrationResponse pResponseStatus_
  = CompleteMigrationResponse'{_cmrsReplicationGroup =
                                 Nothing,
...
instance NFData CompleteMigrationResponse where
