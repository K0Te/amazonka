...
data ModifyUser = ModifyUser'{_muAppendAccessString
                              :: !(Maybe Text),
...
modifyUser
    ...
    -> ModifyUser
modifyUser pUserId_
  = ModifyUser'{_muAppendAccessString = Nothing,
                _muAccessString = Nothing, _muPasswords = Nothing,
...
instance AWSRequest ModifyUser where
        ...
        response
          = receiveXMLWrapper "ModifyUserResult"
              (\ s h x -> parseXML x)

instance Hashable ModifyUser where

instance NFData ModifyUser where
