...
data ValidationErrorsEntry = ValidationErrorsEntry'{_veeCheckName
                                                    :: !(Maybe Text),
                                                    _veeErrorMessage ::
                                                    !(Maybe Text)}
...
validationErrorsEntry
    :: ValidationErrorsEntry
validationErrorsEntry
  = ValidationErrorsEntry'{_veeCheckName = Nothing,
...
instance FromJSON ValidationErrorsEntry where
        parseJSON
          = withObject "ValidationErrorsEntry"
              (\ x ->
                 ValidationErrorsEntry' <$>
                   (x .:? "CheckName") <*> (x .:? "ErrorMessage"))

instance Hashable ValidationErrorsEntry where

instance NFData ValidationErrorsEntry where
