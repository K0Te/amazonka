...
data ParallelDataConfig = ParallelDataConfig'{_pdcS3URI
                                              :: !Text,
...
parallelDataConfig
    :: Text -- ^ 'pdcS3URI'
    -> ParallelDataFormat -- ^ 'pdcFormat'
    -> ParallelDataConfig
parallelDataConfig pS3URI_ pFormat_
  = ParallelDataConfig'{_pdcS3URI = pS3URI_,
                        _pdcFormat = pFormat_}
...
instance FromJSON ParallelDataConfig where
        parseJSON
          = withObject "ParallelDataConfig"
              (\ x ->
                 ParallelDataConfig' <$>
                   (x .: "S3Uri") <*> (x .: "Format"))

instance Hashable ParallelDataConfig where

instance NFData ParallelDataConfig where
