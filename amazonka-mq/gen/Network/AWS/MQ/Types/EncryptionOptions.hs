...
data EncryptionOptions = EncryptionOptions'{_eoKMSKeyId
                                            :: !(Maybe Text),
...
encryptionOptions
    :: Bool -- ^ 'eoUseAWSOwnedKey'
    -> EncryptionOptions
encryptionOptions pUseAWSOwnedKey_
  = EncryptionOptions'{_eoKMSKeyId = Nothing,
...
instance FromJSON EncryptionOptions where
        parseJSON
          = withObject "EncryptionOptions"
              (\ x ->
                 EncryptionOptions' <$>
                   (x .:? "kmsKeyId") <*> (x .: "useAwsOwnedKey"))

instance Hashable EncryptionOptions where

instance NFData EncryptionOptions where
