...
data StartReplay = StartReplay'{_srDescription ::
                                !(Maybe Text),
                                _srReplayName :: !Text,
...
startReplay
    :: Text -- ^ 'srReplayName'
    -> Text -- ^ 'srEventSourceARN'
    -> UTCTime -- ^ 'srEventStartTime'
    -> UTCTime -- ^ 'srEventEndTime'
    -> ReplayDestination -- ^ 'srDestination'
    -> StartReplay
startReplay pReplayName_ pEventSourceARN_
  pEventStartTime_ pEventEndTime_ pDestination_
  = StartReplay'{_srDescription = Nothing,
...
instance AWSRequest StartReplay where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 StartReplayResponse' <$>
                   (x .?> "State") <*> (x .?> "ReplayStartTime") <*>
                     (x .?> "ReplayArn")
                     <*> (x .?> "StateReason")
                     <*> (pure (fromEnum s)))

instance Hashable StartReplay where

instance NFData StartReplay where
...
data StartReplayResponse = StartReplayResponse'{_srrsState
                                                :: !(Maybe ReplayState),
                                                _srrsReplayStartTime ::
                                                !(Maybe POSIX),
...
startReplayResponse pResponseStatus_
  = StartReplayResponse'{_srrsState = Nothing,
                         _srrsReplayStartTime = Nothing,
                         _srrsReplayARN = Nothing, _srrsStateReason = Nothing,
...
instance NFData StartReplayResponse where
