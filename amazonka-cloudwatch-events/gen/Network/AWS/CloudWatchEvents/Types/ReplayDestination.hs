...
data ReplayDestination = ReplayDestination'{_rdFilterARNs
                                            :: !(Maybe [Text]),
                                            _rdARN :: !Text}
...
replayDestination pARN_
  = ReplayDestination'{_rdFilterARNs = Nothing,
                       _rdARN = pARN_}
...
instance FromJSON ReplayDestination where
        parseJSON
          = withObject "ReplayDestination"
              (\ x ->
                 ReplayDestination' <$>
                   (x .:? "FilterArns" .!= mempty) <*> (x .: "Arn"))

instance Hashable ReplayDestination where

instance NFData ReplayDestination where
