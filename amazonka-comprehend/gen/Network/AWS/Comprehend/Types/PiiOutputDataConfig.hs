...
data PiiOutputDataConfig = PiiOutputDataConfig'{_podcKMSKeyId
                                                :: !(Maybe Text),
                                                _podcS3URI :: !Text}
...
piiOutputDataConfig pS3URI_
  = PiiOutputDataConfig'{_podcKMSKeyId = Nothing,
                         _podcS3URI = pS3URI_}
...
instance FromJSON PiiOutputDataConfig where
        parseJSON
          = withObject "PiiOutputDataConfig"
              (\ x ->
                 PiiOutputDataConfig' <$>
                   (x .:? "KmsKeyId") <*> (x .: "S3Uri"))

instance Hashable PiiOutputDataConfig where

instance NFData PiiOutputDataConfig where
