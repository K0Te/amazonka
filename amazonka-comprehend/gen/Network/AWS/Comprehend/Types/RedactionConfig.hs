...
data RedactionConfig = RedactionConfig'{_rcMaskCharacter
                                        :: !(Maybe Text),
                                        _rcMaskMode ::
...
redactionConfig
    :: RedactionConfig
redactionConfig
  = RedactionConfig'{_rcMaskCharacter = Nothing,
                     _rcMaskMode = Nothing, _rcPiiEntityTypes = Nothing}
...
instance FromJSON RedactionConfig where
        parseJSON
          = withObject "RedactionConfig"
              (\ x ->
                 RedactionConfig' <$>
                   (x .:? "MaskCharacter") <*> (x .:? "MaskMode") <*>
                     (x .:? "PiiEntityTypes" .!= mempty))

instance Hashable RedactionConfig where

instance NFData RedactionConfig where
