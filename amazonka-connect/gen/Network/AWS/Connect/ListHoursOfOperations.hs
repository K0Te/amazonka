...
instance AWSPager ListHoursOfOperations where
        page rq rs
          ...
          | stop (rs ^. lhoorsHoursOfOperationSummaryList) =
            Nothing
          | otherwise =
            Just $ rq & lhooNextToken .~ rs ^. lhoorsNextToken

instance AWSRequest ListHoursOfOperations where
        type Rs ListHoursOfOperations =
             ListHoursOfOperationsResponse
        request = get connect
        response
          = receiveJSON
...
instance Hashable ListHoursOfOperations where

instance NFData ListHoursOfOperations where
...
instance NFData ListHoursOfOperationsResponse where
