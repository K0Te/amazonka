...
data ListLexBots = ListLexBots'{_llbNextToken ::
                                !(Maybe Text),
...
listLexBots pInstanceId_
  = ListLexBots'{_llbNextToken = Nothing,
...
instance AWSPager ListLexBots where
        page rq rs
          ...
          | otherwise =
            Just $ rq & llbNextToken .~ rs ^. llbrsNextToken
...
instance AWSRequest ListLexBots where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 ListLexBotsResponse' <$>
                   (x .?> "NextToken") <*> (x .?> "LexBots" .!@ mempty)
                     <*> (pure (fromEnum s)))

instance Hashable ListLexBots where

instance NFData ListLexBots where
...
data ListLexBotsResponse = ListLexBotsResponse'{_llbrsNextToken
                                                :: !(Maybe Text),
                                                _llbrsLexBots ::
                                                !(Maybe [LexBot]),
...
listLexBotsResponse
    :: Int -- ^ 'llbrsResponseStatus'
    -> ListLexBotsResponse
listLexBotsResponse pResponseStatus_
  = ListLexBotsResponse'{_llbrsNextToken = Nothing,
...
instance NFData ListLexBotsResponse where
