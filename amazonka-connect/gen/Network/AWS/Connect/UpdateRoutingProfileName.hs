...
data UpdateRoutingProfileName = UpdateRoutingProfileName'{_urpnName
                                                          :: !(Maybe Text),
                                                          _urpnDescription ::
                                                          !(Maybe Text),
                                                          _urpnInstanceId ::
...
updateRoutingProfileName
    :: Text -- ^ 'urpnInstanceId'
    -> Text -- ^ 'urpnRoutingProfileId'
    -> UpdateRoutingProfileName
updateRoutingProfileName pInstanceId_
  pRoutingProfileId_
  = UpdateRoutingProfileName'{_urpnName = Nothing,
...
instance AWSRequest UpdateRoutingProfileName where
        type Rs UpdateRoutingProfileName =
             UpdateRoutingProfileNameResponse
        request = postJSON connect
        response
          = receiveNull UpdateRoutingProfileNameResponse'

instance Hashable UpdateRoutingProfileName where

instance NFData UpdateRoutingProfileName where
...
instance ToPath UpdateRoutingProfileName where
        toPath UpdateRoutingProfileName'{..}
          = mconcat
              ["/routing-profiles/", toBS _urpnInstanceId, "/",
               toBS _urpnRoutingProfileId, "/name"]
