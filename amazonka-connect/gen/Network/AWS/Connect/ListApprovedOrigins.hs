...
data ListApprovedOrigins = ListApprovedOrigins'{_laoNextToken
                                                :: !(Maybe Text),
...
listApprovedOrigins pInstanceId_
  = ListApprovedOrigins'{_laoNextToken = Nothing,
...
instance AWSPager ListApprovedOrigins where
        page rq rs
          ...
          | otherwise =
            Just $ rq & laoNextToken .~ rs ^. laorsNextToken

instance AWSRequest ListApprovedOrigins where
        type Rs ListApprovedOrigins =
             ListApprovedOriginsResponse
        request = get connect
        response
          = receiveJSON
              (\ s h x ->
                 ListApprovedOriginsResponse' <$>
                   (x .?> "NextToken") <*> (x .?> "Origins" .!@ mempty)
                     <*> (pure (fromEnum s)))

instance Hashable ListApprovedOrigins where

instance NFData ListApprovedOrigins where
...
data ListApprovedOriginsResponse = ListApprovedOriginsResponse'{_laorsNextToken
                                                                ::
                                                                !(Maybe Text),
                                                                _laorsOrigins ::
                                                                !(Maybe [Text]),
                                                                _laorsResponseStatus
                                                                :: !Int}
...
listApprovedOriginsResponse
    :: Int -- ^ 'laorsResponseStatus'
    -> ListApprovedOriginsResponse
listApprovedOriginsResponse pResponseStatus_
  = ListApprovedOriginsResponse'{_laorsNextToken =
                                   Nothing,
...
instance NFData ListApprovedOriginsResponse where
