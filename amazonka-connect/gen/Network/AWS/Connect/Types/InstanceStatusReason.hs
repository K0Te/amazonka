...
newtype InstanceStatusReason = InstanceStatusReason'{_isrMessage
                                                     :: Maybe Text}
...
instance FromJSON InstanceStatusReason where
        parseJSON
          = withObject "InstanceStatusReason"
              (\ x -> InstanceStatusReason' <$> (x .:? "Message"))

instance Hashable InstanceStatusReason where

instance NFData InstanceStatusReason where
