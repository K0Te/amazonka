...
data PromptSummary = PromptSummary'{_psARN ::
                                    !(Maybe Text),
                                    _psName :: !(Maybe Text),
                                    _psId :: !(Maybe Text)}
...
promptSummary
    :: PromptSummary
promptSummary
  = PromptSummary'{_psARN = Nothing, _psName = Nothing,
                   _psId = Nothing}
...
instance FromJSON PromptSummary where
        parseJSON
          = withObject "PromptSummary"
              (\ x ->
                 PromptSummary' <$>
                   (x .:? "Arn") <*> (x .:? "Name") <*> (x .:? "Id"))

instance Hashable PromptSummary where

instance NFData PromptSummary where
