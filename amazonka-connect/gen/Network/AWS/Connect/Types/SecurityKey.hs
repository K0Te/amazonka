...
data SecurityKey = SecurityKey'{_skCreationTime ::
                                !(Maybe POSIX),
...
securityKey
    :: SecurityKey
securityKey
  = SecurityKey'{_skCreationTime = Nothing,
                 _skAssociationId = Nothing, _skKey = Nothing}
...
instance FromJSON SecurityKey where
        parseJSON
          = withObject "SecurityKey"
              (\ x ->
                 SecurityKey' <$>
                   (x .:? "CreationTime") <*> (x .:? "AssociationId")
                     <*> (x .:? "Key"))

instance Hashable SecurityKey where

instance NFData SecurityKey where
