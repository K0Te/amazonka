...
data MediaConcurrency = MediaConcurrency'{_mcChannel
                                          :: !Channel,
                                          _mcConcurrency :: !Nat}
...
mediaConcurrency
    :: Channel -- ^ 'mcChannel'
    -> Natural -- ^ 'mcConcurrency'
    -> MediaConcurrency
mediaConcurrency pChannel_ pConcurrency_
  = MediaConcurrency'{_mcChannel = pChannel_,
...
instance FromJSON MediaConcurrency where
        parseJSON
          = withObject "MediaConcurrency"
              (\ x ->
                 MediaConcurrency' <$>
                   (x .: "Channel") <*> (x .: "Concurrency"))

instance Hashable MediaConcurrency where

instance NFData MediaConcurrency where
