...
data LexBot = LexBot'{_lbLexRegion :: !(Maybe Text),
                      _lbName :: !(Maybe Text)}
...
lexBot
    :: LexBot
lexBot
  = LexBot'{_lbLexRegion = Nothing, _lbName = Nothing}
...
instance FromJSON LexBot where
        parseJSON
          = withObject "LexBot"
              (\ x ->
                 LexBot' <$> (x .:? "LexRegion") <*> (x .:? "Name"))

instance Hashable LexBot where

instance NFData LexBot where
