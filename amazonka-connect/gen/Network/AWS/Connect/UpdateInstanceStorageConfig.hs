...
data UpdateInstanceStorageConfig = UpdateInstanceStorageConfig'{_uiscInstanceId
                                                                :: !Text,
                                                                _uiscAssociationId
                                                                :: !Text,
                                                                _uiscResourceType
...
updateInstanceStorageConfig
    :: Text -- ^ 'uiscInstanceId'
    -> Text -- ^ 'uiscAssociationId'
    -> InstanceStorageResourceType -- ^ 'uiscResourceType'
    -> InstanceStorageConfig -- ^ 'uiscStorageConfig'
    -> UpdateInstanceStorageConfig
updateInstanceStorageConfig pInstanceId_
  pAssociationId_ pResourceType_ pStorageConfig_
  = UpdateInstanceStorageConfig'{_uiscInstanceId =
                                   pInstanceId_,
...
instance AWSRequest UpdateInstanceStorageConfig where
        type Rs UpdateInstanceStorageConfig =
             UpdateInstanceStorageConfigResponse
        request = postJSON connect
        response
          = receiveNull UpdateInstanceStorageConfigResponse'

instance Hashable UpdateInstanceStorageConfig where

instance NFData UpdateInstanceStorageConfig where
...
instance ToPath UpdateInstanceStorageConfig where
        toPath UpdateInstanceStorageConfig'{..}
          = mconcat
              ["/instance/", toBS _uiscInstanceId,
               "/storage-config/", toBS _uiscAssociationId]
