...
newtype CacheAttributes = CacheAttributes'{_caCacheStaleTimeoutInSeconds
                                           :: Maybe Int}
...
cacheAttributes
    :: CacheAttributes
cacheAttributes
  = CacheAttributes'{_caCacheStaleTimeoutInSeconds =
                       Nothing}
...
instance FromJSON CacheAttributes where
        parseJSON
          = withObject "CacheAttributes"
              (\ x ->
                 CacheAttributes' <$>
                   (x .:? "CacheStaleTimeoutInSeconds"))

instance Hashable CacheAttributes where

instance NFData CacheAttributes where
