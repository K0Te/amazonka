...
data ListTapePools = ListTapePools'{_ltpPoolARNs ::
                                    !(Maybe [Text]),
...
listTapePools
    :: ListTapePools
listTapePools
  = ListTapePools'{_ltpPoolARNs = Nothing,
                   _ltpMarker = Nothing, _ltpLimit = Nothing}
...
instance AWSPager ListTapePools where
        page rq rs
          ...
          | otherwise =
            Just $ rq & ltpMarker .~ rs ^. ltprsMarker
...
instance AWSRequest ListTapePools where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 ListTapePoolsResponse' <$>
                   (x .?> "PoolInfos" .!@ mempty) <*> (x .?> "Marker")
                     <*> (pure (fromEnum s)))

instance Hashable ListTapePools where

instance NFData ListTapePools where
...
data ListTapePoolsResponse = ListTapePoolsResponse'{_ltprsPoolInfos
                                                    :: !(Maybe [PoolInfo]),
                                                    _ltprsMarker ::
                                                    !(Maybe Text),
...
listTapePoolsResponse pResponseStatus_
  = ListTapePoolsResponse'{_ltprsPoolInfos = Nothing,
...
instance NFData ListTapePoolsResponse where
