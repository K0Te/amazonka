...
newtype DescribeBandwidthRateLimitSchedule = DescribeBandwidthRateLimitSchedule'{_dbrlsGatewayARN
                                                                                 ::
                                                                                 Text}
...
describeBandwidthRateLimitSchedule
    :: Text -- ^ 'dbrlsGatewayARN'
    -> DescribeBandwidthRateLimitSchedule
describeBandwidthRateLimitSchedule pGatewayARN_
  = DescribeBandwidthRateLimitSchedule'{_dbrlsGatewayARN
                                          = pGatewayARN_}
...
instance ToHeaders DescribeBandwidthRateLimitSchedule
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON DescribeBandwidthRateLimitSchedule
         where
        toJSON DescribeBandwidthRateLimitSchedule'{..}
          = object
              (catMaybes [Just ("GatewayARN" .= _dbrlsGatewayARN)])

instance ToPath DescribeBandwidthRateLimitSchedule
         where
        toPath = const "/"

instance ToQuery DescribeBandwidthRateLimitSchedule
         where
        toQuery = const mempty

-- | /See:/ 'describeBandwidthRateLimitScheduleResponse' smart constructor.
data DescribeBandwidthRateLimitScheduleResponse = DescribeBandwidthRateLimitScheduleResponse'{_dbrlsrsGatewayARN
                                                                                              ::
                                                                                              !(Maybe
                                                                                                  Text),
...
describeBandwidthRateLimitScheduleResponse
    :: Int -- ^ 'dbrlsrsResponseStatus'
    -> DescribeBandwidthRateLimitScheduleResponse
describeBandwidthRateLimitScheduleResponse
  pResponseStatus_
  = DescribeBandwidthRateLimitScheduleResponse'{_dbrlsrsGatewayARN
                                                  = Nothing,
                                                _dbrlsrsBandwidthRateLimitIntervals
                                                  = Nothing,
                                                _dbrlsrsResponseStatus =
