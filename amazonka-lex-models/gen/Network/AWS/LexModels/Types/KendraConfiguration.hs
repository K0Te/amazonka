...
data KendraConfiguration = KendraConfiguration'{_kcQueryFilterString
                                                :: !(Maybe Text),
                                                _kcKendraIndex :: !Text,
                                                _kcRole :: !Text}
...
kendraConfiguration pKendraIndex_ pRole_
  = KendraConfiguration'{_kcQueryFilterString =
                           Nothing,
                         _kcKendraIndex = pKendraIndex_, _kcRole = pRole_}
...
instance FromJSON KendraConfiguration where
        parseJSON
          = withObject "KendraConfiguration"
              (\ x ->
                 KendraConfiguration' <$>
                   (x .:? "queryFilterString") <*> (x .: "kendraIndex")
                     <*> (x .: "role"))

instance Hashable KendraConfiguration where

instance NFData KendraConfiguration where
