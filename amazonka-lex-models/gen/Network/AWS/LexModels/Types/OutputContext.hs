...
data OutputContext = OutputContext'{_ocName :: !Text,
...
outputContext
    :: Text -- ^ 'ocName'
    -> Natural -- ^ 'ocTimeToLiveInSeconds'
    -> Natural -- ^ 'ocTurnsToLive'
    -> OutputContext
outputContext pName_ pTimeToLiveInSeconds_
  pTurnsToLive_
  = OutputContext'{_ocName = pName_,
...
instance FromJSON OutputContext where
        parseJSON
          = withObject "OutputContext"
              (\ x ->
                 OutputContext' <$>
                   (x .: "name") <*> (x .: "timeToLiveInSeconds") <*>
                     (x .: "turnsToLive"))

instance Hashable OutputContext where

instance NFData OutputContext where
