...
data KeyGroupConfig = KeyGroupConfig'{_kgcComment ::
                                      !(Maybe Text),
                                      _kgcName :: !Text, _kgcItems :: ![Text]}
...
keyGroupConfig pName_
  = KeyGroupConfig'{_kgcComment = Nothing,
                    _kgcName = pName_, _kgcItems = mempty}
...
instance Hashable KeyGroupConfig where

instance NFData KeyGroupConfig where

instance ToXML KeyGroupConfig where
        toXML KeyGroupConfig'{..}
          = mconcat
              ["Comment" @= _kgcComment, "Name" @= _kgcName,
