...
instance AWSRequest SelectAggregateResourceConfig
         where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 SelectAggregateResourceConfigResponse' <$>
                   (x .?> "Results" .!@ mempty) <*> (x .?> "QueryInfo")
                     <*> (x .?> "NextToken")
                     <*> (pure (fromEnum s)))

instance Hashable SelectAggregateResourceConfig where

instance NFData SelectAggregateResourceConfig where

instance ToHeaders SelectAggregateResourceConfig
         where
        toHeaders
          = const
              (mconcat
