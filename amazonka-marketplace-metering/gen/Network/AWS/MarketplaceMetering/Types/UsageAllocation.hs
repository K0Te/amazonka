...
data UsageAllocation = UsageAllocation'{_uaTags ::
                                        !(Maybe (List1 Tag)),
...
usageAllocation
    :: Natural -- ^ 'uaAllocatedUsageQuantity'
    -> UsageAllocation
usageAllocation pAllocatedUsageQuantity_
  = UsageAllocation'{_uaTags = Nothing,
...
instance FromJSON UsageAllocation where
        parseJSON
          = withObject "UsageAllocation"
              (\ x ->
                 UsageAllocation' <$>
                   (x .:? "Tags") <*> (x .: "AllocatedUsageQuantity"))

instance Hashable UsageAllocation where

instance NFData UsageAllocation where
