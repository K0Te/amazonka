...
data ManagedScaling = ManagedScaling'{_msStatus ::
...
managedScaling
    :: ManagedScaling
managedScaling
  = ManagedScaling'{_msStatus = Nothing,
...
instance FromJSON ManagedScaling where
        parseJSON
          = withObject "ManagedScaling"
              (\ x ->
                 ManagedScaling' <$>
                   (x .:? "status") <*> (x .:? "maximumScalingStepSize")
                     <*> (x .:? "targetCapacity")
                     <*> (x .:? "minimumScalingStepSize")
                     <*> (x .:? "instanceWarmupPeriod"))

instance Hashable ManagedScaling where

instance NFData ManagedScaling where
