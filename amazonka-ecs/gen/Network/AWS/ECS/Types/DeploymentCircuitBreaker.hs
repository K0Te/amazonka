...
data DeploymentCircuitBreaker = DeploymentCircuitBreaker'{_dcbEnable
                                                          :: !Bool,
                                                          _dcbRollback :: !Bool}
...
deploymentCircuitBreaker pEnable_ pRollback_
  = DeploymentCircuitBreaker'{_dcbEnable = pEnable_,
...
instance FromJSON DeploymentCircuitBreaker where
        parseJSON
          = withObject "DeploymentCircuitBreaker"
              (\ x ->
                 DeploymentCircuitBreaker' <$>
                   (x .: "enable") <*> (x .: "rollback"))

instance Hashable DeploymentCircuitBreaker where

instance NFData DeploymentCircuitBreaker where
