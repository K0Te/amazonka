...
data EnvironmentFile = EnvironmentFile'{_efValue ::
                                        !Text,
...
environmentFile
    :: Text -- ^ 'efValue'
    -> EnvironmentFileType -- ^ 'efType'
    -> EnvironmentFile
environmentFile pValue_ pType_
  = EnvironmentFile'{_efValue = pValue_,
                     _efType = pType_}
...
instance FromJSON EnvironmentFile where
        parseJSON
          = withObject "EnvironmentFile"
              (\ x ->
                 EnvironmentFile' <$>
                   (x .: "value") <*> (x .: "type"))

instance Hashable EnvironmentFile where

instance NFData EnvironmentFile where
