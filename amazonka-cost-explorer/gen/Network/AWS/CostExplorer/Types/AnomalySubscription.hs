...
data AnomalySubscription = AnomalySubscription'{_asAccountId
                                                :: !(Maybe Text),
                                                _asSubscriptionARN ::
                                                !(Maybe Text),
...
anomalySubscription
    :: Double -- ^ 'asThreshold'
    -> AnomalySubscriptionFrequency -- ^ 'asFrequency'
    -> Text -- ^ 'asSubscriptionName'
    -> AnomalySubscription
anomalySubscription pThreshold_ pFrequency_
  pSubscriptionName_
  = AnomalySubscription'{_asAccountId = Nothing,
                         _asSubscriptionARN = Nothing,
                         _asMonitorARNList = mempty, _asSubscribers = mempty,
...
instance FromJSON AnomalySubscription where
        parseJSON
          = withObject "AnomalySubscription"
              (\ x ->
                 AnomalySubscription' <$>
                   (x .:? "AccountId") <*> (x .:? "SubscriptionArn") <*>
...
instance Hashable AnomalySubscription where

instance NFData AnomalySubscription where
