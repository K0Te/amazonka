...
data RootCause = RootCause'{_rcService ::
                            !(Maybe Text),
...
rootCause
    :: RootCause
rootCause
  = RootCause'{_rcService = Nothing,
               _rcUsageType = Nothing, _rcLinkedAccount = Nothing,
               _rcRegion = Nothing}
...
instance FromJSON RootCause where
        parseJSON
          = withObject "RootCause"
              (\ x ->
                 RootCause' <$>
                   (x .:? "Service") <*> (x .:? "UsageType") <*>
                     (x .:? "LinkedAccount")
                     <*> (x .:? "Region"))

instance Hashable RootCause where

instance NFData RootCause where
