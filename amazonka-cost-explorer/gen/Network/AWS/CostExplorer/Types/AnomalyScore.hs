...
data AnomalyScore = AnomalyScore'{_asMaxScore ::
                                  !Double,
...
anomalyScore
    :: Double -- ^ 'asMaxScore'
    -> Double -- ^ 'asCurrentScore'
    -> AnomalyScore
anomalyScore pMaxScore_ pCurrentScore_
  = AnomalyScore'{_asMaxScore = pMaxScore_,
...
instance FromJSON AnomalyScore where
        parseJSON
          = withObject "AnomalyScore"
              (\ x ->
                 AnomalyScore' <$>
                   (x .: "MaxScore") <*> (x .: "CurrentScore"))

instance Hashable AnomalyScore where

instance NFData AnomalyScore where
