...
data MySQLSettings = MySQLSettings'{_msqlsMaxFileSize
                                    :: !(Maybe Int),
...
instance FromJSON MySQLSettings where
        parseJSON
          = withObject "MySQLSettings"
              (\ x ->
                 MySQLSettings' <$>
                   (x .:? "MaxFileSize") <*> (x .:? "TargetDbType") <*>
                     (x .:? "ServerName")
...
instance Hashable MySQLSettings where

instance NFData MySQLSettings where
