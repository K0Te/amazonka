...
newtype TracingConfiguration = TracingConfiguration'{_tcEnabled
                                                     :: Maybe Bool}
...
instance FromJSON TracingConfiguration where
        parseJSON
          = withObject "TracingConfiguration"
              (\ x -> TracingConfiguration' <$> (x .:? "enabled"))

instance Hashable TracingConfiguration where

instance NFData TracingConfiguration where
